  ...
  switch(event)
  {
    case IPC_ROUTER_EVENT_RESUME_TX:
    {
      ...
      if(dest)
      {
        ...
        {
          /* try transmitting */
          if(ipc_router_qdi_send(client, dest->addr, buf->msg, buf->len) 
 ...
 )
{
  ...
  if(xp)
  {
    ...
    xp->sig = *os_params;
    if(ipc_router_qdi_open(&xp->handle, 0, qmi_csi_rx_cb, xp, qmi_csi_event_cb,
 ...
 )
{
  ...
  rc = ipc_router_qdi_reg_server(xp->handle, &service);
 ...
 )
{
  ...
  rc = ipc_router_qdi_dereg_server(xp->handle, &service);
  if(rc != IPC_ROUTER_STATUS_SUCCESS)
  {
    return QMI_CSI_INTERNAL_ERR;
  }
 ...
 )
{
  ...
  {
    rc = ipc_router_qdi_send(xp->handle, addr, msg, len);
 ...
 )
{
  ...
  while((buf = dequeue_rx(xp)) != NULL)
  {
    ...
    else
    {
      if(buf->confirm_rx)
      {
        ipc_router_qdi_send_conf(xp->handle, &buf->src_addr);
      }
