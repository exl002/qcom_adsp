...
#define QMI_CSI_OS_LOG_TX_EXT(header, svc_id, addr, addr_len) QMI_CSI_OS_LOG_TX(header)
#endif

#ifndef QMI_CSI_OS_LOG_RX_EXT
#define QMI_CSI_OS_LOG_RX_EXT(header, svc_id, addr, addr_len) QMI_CSI_OS_LOG_RX(header)
#endif
...
)
{
  ...
  if(clnt)
  {
    QMI_CSI_OS_LOG_TX_EXT(msg, svc->service_obj->service_id, clnt->xport.addr, clnt->xport.xport->addr_len);
...
)
{
  ...
  QMI_CSI_OS_LOG_TX_EXT(msg, xport->service->service_obj->service_id, addr, xport->addr_len);
 ...
 )
{
  ...
  qmi_csi_process_req process_request = NULL;

  if(!xport || !xport->service || !addr || len < QMI_HEADER_SIZE)
    return QMI_CSI_INTERNAL_ERR;

  svc = xport->service; 
  process_request = svc->service_process_req;

  QMI_CSI_OS_LOG_RX_EXT(buf, svc->service_obj->service_id, addr, xport->addr_len);
  ...
  if(!clnt)
  {
    ...
    if(rc != QMI_CSI_NO_ERR)
    {
      encode_and_send_resp(xport, addr, msg_id, txn_id, 
          QMI_RESULT_FAILURE_V01, qmi_csi_err_translate(rc));
  ...
  {
    c_struct_len = len - QMI_HEADER_SIZE;
    if(c_struct_len)
      c_struct = buf + QMI_HEADER_SIZE;
    process_request = svc->service_process_raw_req;
  }
  ...
  if (msg_id <= QMI_CSI_FW_MSG_HANDLER_MAX_MSG_ID)
  {
    qmi_csi_msg_handler_type *msg_handler = LIST_HEAD(msg_handler_list);
    while (msg_handler)
    {
    ...
    if (msg_handler && msg_handler->msg_handler(svc->service_obj,GET_INSTANCE(svc->idl_version),msg_id,
                                      (qmi_req_handle)(uintptr_t)txn_handle,
                                      (void*)c_struct,c_struct_len) == QMI_CSI_NO_ERR)
    {
      if (c_struct && c_struct != (buf + QMI_HEADER_SIZE))
      {
        FREE(c_struct);
      }
      return QMI_CSI_NO_ERR;
  ...
  {
    /* find and remove transaction - txn cannot be trusted because it can be
     * freed during the callback
     */
    LOCK(&txn_list_lock);
    ...
    UNLOCK(&txn_list_lock);
    encode_and_send_resp(xport, addr, msg_id, txn_id, QMI_RESULT_FAILURE_V01,
 ...
 )
{
  ...
  *service_provider = NULL;
  ...
  if(options)
  {
#ifdef QMI_CSI_OPTIONS_INSTANCE_ID_VALID
...
#ifdef QMI_CSI_OPTIONS_SCOPE_VALID
