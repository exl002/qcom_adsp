...
static void send_indications(ind_type *params)
{
  ...
  if(!params || !params->clnt)
  {
    return;
  }

  /* Freed at the end of this function */
  ind = (test_data_ind_msg_v01*)MALLOC(sizeof(test_data_ind_msg_v01));
  if(!ind)
  {
    // Send indications
    return;
  ...
  for(i=0;i<num_inds;++i)
  {
    ...
    resp_err = qmi_csi_send_ind( params->clnt->clnt, QMI_TEST_DATA_IND_V01, ind, sizeof(test_data_ind_msg_v01) );
    if(resp_err != QMI_CSI_NO_ERR)
    {
