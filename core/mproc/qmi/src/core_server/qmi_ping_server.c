...
$Header: //components/rel/core.adsp/2.2/mproc/qmi/src/core_server/qmi_ping_server.c#1 $
                          ...
                          uint32_t           req_c_struct_len)
{
   ...
   {
     memcpy(resp_msg->data, recv_msg->data, resp_msg->data_len);
                ...
                ping_server_error       *err)
{
    ...
    /*Construct and Initialize the core server object */
    rc = (ping_server_error)qmi_core_server_new(&(ping_server_object->core_object),
                             object_name,
               instance_id,/* Instance id */
                             1,/* Task flag */
                             (void*)ping_server_event_loop,
              ...
              uint32_t  ind_c_struct_len)
{
  ...
  if (!ind_obj || !ind_c_struct)
   ...
   base_ind =  (qmi_indication_type*)ind_obj;
   ...
   for(i = 0;i < ind_data->num_inds; i++ )
   {

     qmi_indication_send(base_ind,
                               ind_data_to_send,
                               ind_c_struct_len);
     if(ind_data->ind_delay > 0)
     {
       /* TODO msleep(ind_data->ind_delay) */
     }
