/******************************************************************************
  ...
  QUALCOMM Propritargetetary and Confidential.
  ---------------------------------------------------------------------------
*******************************************************************************/
#include <string.h>
 ...
 )
{
  ...
  /* linearize buffer and free packet */
...
{
  return (GET_VERSION(inst1) == GET_VERSION(inst2));
}
 ...
 )
{
  ...
  if(ipc_router_core_find_all_servers(NULL, &service, servers, 
        (uint32 *)num_entries, (uint32 *)&num_servers, compare, NULL) 
  ...
  if(num_entries)
  {
    for(i = 0; i < *num_entries; i++)
    {
      ...
      memcpy(&service_info[i].addr, &servers[i].addr, 
