...
$Header: //components/rel/core.adsp/2.2/mproc/ipc_router/src/ipc_router_debug.c#2 $
...
{   
  ...
  while(NULL != (server_list = LIST_HEAD(results.servers)))
  {
    IPC_ROUTER_OS_MESSAGE(IPC_ROUTER_MSG_INFO,"\t(0x%x,0x%x)",
        server_list->service.type, server_list->service.instance, 0);
    IPC_ROUTER_OS_MESSAGE(IPC_ROUTER_MSG_INFO,"\t @ 0x%x:0x%x\n",
        server_list->addr.processor_id, server_list->addr.port_id, 0);
