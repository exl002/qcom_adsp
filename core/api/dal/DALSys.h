...
$Header: //components/rel/core.adsp/2.2/api/dal/DALSys.h#7 $
=============================================================================*/
...
/*
  ...
  A DALSYS memory object represents a MMU mapping between virtual and physical
  memory, as well as the permissions and attributes for accessing this memory.

  This memory object is typically created for a specified physical memory 
  region, such as HWIO registers or shared memory, and an unspecified 
  virtual memory region to be assigned by DAL.  

  On memory object creation, DAL checks if there exists a static mapping with 
  compatible attributes.  If so the memory object is created for the existing
  static mapping.  If not a dynamic mapping is created.

  All lengths and addresses provided by users may be adjusted to the 
  natural boundaries in the underlying architecture.

  Users have the option of pre-allocating memory required for this object. 
  This means the users provide an valid virtual memory region that is mapped 
  by MMU to a valid physical memory region.  
  The DALSYS_MEM_PROPS_PHYS_PREALLOC attribute is used to denote this usage.

  This API uses 32-bit physical address.
...
/*
  ...
  This API uses 64-bit physical address.

  @param pMemInfo    : Includes virtual start address, physical start address, 
                       memory attributes, and region length
                       DALSYS_MEM_ADDR_NOT_SPECIFIED should be used
                       for unspecified address
                       Attributes include cacheability, permissions, and
                       preallocation
                       This is updated with correct information upon
...
/**
  @brief Get the DAL Properties handle from dynamically loaded config module
