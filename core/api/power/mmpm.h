...
$Header: //components/rel/core.adsp/2.2/api/power/mmpm.h#4 $
...
/** @endcond */
    ...
    /**< Sets memory address mapping for specified memory*/
...
/**
  ...
  Pointer to this structure is used in pParamConfig for paramId = MMPM_PARAM_ID_MEMORY_MAP;
*/
typedef struct{
    MmpmMemIdType memory;              /**< Memory. */
    uint64  startAddress;              /**< Virual address of the start of memory range. */
    uint32  size;                      /**< Size of the memory range in bytes @newpagetable*/
}MmpmMemoryMapType;
...
/** 
  ...
  The paramId :
  MMPM_PARAM_ID_CLIENT_OCMEM_MAP - pParamConfig points to the
  #MmpmOcmemMapType structure.  
  MMPM_PARAM_ID_MEMORY_MAP - pParamConfig points to the
  #MmpmMemoryMapType structure.
...
/**
  On-chip memory map region description.
*/
typedef struct{
    uint32 regionId;
    /**< Region ID. */
...
typedef struct{
    ...
    /**< Array with all regions for the allocation. @newpagetable */
} MmpmOcmemMapType;
/** @} */ /* end_addtogroup mmpm_datatypes */
...
/** @ingroup mmpm_func_set_parameter
  Sets various client-specific and global parameters. Currently, this function 
  only supports setting the client OCMEM allocation memory map.
