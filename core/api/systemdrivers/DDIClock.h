...
/*
  ...
  $Header: //components/rel/core.adsp/2.2/api/systemdrivers/DDIClock.h#4 $
  $DateTime: 2013/05/14 06:03:09 $
  $Author: coresvc $
...
/**
  ...
  at the start of the Sleep sequence.

  @param _h            [in]  Handle to the DAL device.
  @param bRequired     [in]  Flag specifying if the ramping is required.  A
                             false value will re-aggregate the CPU requests.
  @param pnFrequencyHz [out] Pointer to uint32 to fill in with the frequency.
                             A value of zero indicates that the frequency
                             could not be set.
  ...
  DAL_ERROR -- Clock frequency was not set successfully.
