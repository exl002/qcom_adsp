...
def build_qurt_config(dict, build_flags, asic, tools_path, object_path):
    ...
    if tools_path != None:
      if (os.name == 'posix') :
        GCC = "\"" + tools_path + "\"" + "/gnu/bin/hexagon-gcc"
      else:
        GCC = "\"" + tools_path.replace("/", "\\") + "\\gnu\\bin\\hexagon-gcc" + "\""
    else:
      GCC = "hexagon-gcc"     
    
    command_line = GCC + " -g -nostdinc -O2 -m" + asic + " -G0" + " " + build_flags + " -c -o " + obj_file.replace("\\",'/') + " " + src_file.replace("\\",'/')

    print command_line
    import shlex
    gcc_args = shlex.split(command_line)

    s_err = subprocess.Popen(gcc_args).wait()
    if s_err != 0:
      print >> sys.stderr, "\n*** Configuration build failed, error %d" % s_err
      sys.exit(1)
