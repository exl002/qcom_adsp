...
Copyright (c) 1997-2013 by Qualcomm Technologies Incorporated.  All Rights Reserved.
============================================================================ */
...
$Header: //components/rel/core.adsp/2.2/services/utils/src/memheap.c#18 $ $DateTime: 2014/02/11 05:37:40 $ $Author: coresvc $
...
#define TCB_TYPE     osal_tcb_t*
#define TCB_SELF()   osal_thread_self()
...
#define MEMHEAP_MAX_THREADS 1000
typedef struct
{
...
memheap_task_stats_type memheap_task_stats[MEMHEAP_MAX_THREADS];
...
char memheap_tls_to_tcbname[MEMHEAP_MAX_THREADS][QURT_THREAD_ATTR_NAME_MAXLEN];
...
static void memheap_updatestats(mem_heap_type* heap_type, uint32 size, uint32 op, uint32* tls_index)
{
  ...
  if (op == kaddSize)
  {
    index = qurt_thread_get_id();
    index = index&MEMHEAP_THREAD_ID_MASK;
    *tls_index = index;

    if (index < MEMHEAP_MAX_THREADS)
    {
      if (memheap_tls_to_tcbname[index][0] == 0)
      {
        qurt_thread_get_name(&memheap_tls_to_tcbname[index][0],QURT_THREAD_ATTR_NAME_MAXLEN);
      }

      memheap_task_stats[index].current_usage = memheap_task_stats[index].current_usage+size;
    }
  }
  else if (op == ksubSize)
  {
  ...
  if (memheap_task_stats[index].current_usage > memheap_task_stats[index].max_usage)
  {
    memheap_task_stats[index].max_usage = memheap_task_stats[index].current_usage;
  }
  ...
  do {
   ...
   if(heap_ptr->legacy_check & kUseBINsImpl){
    ...
    if (freeBlock)
    {
     ...
     memheap_updatestats(heap_ptr,actualSize,kaddSize,&stats_index);
     freeBlock->stats_index=stats_index;
    ...
    if(answer != NULL)
    {
      ...
      freeBlock->header_debug_info.caller_ptr=MEM_HEAP_CALLER_ADDRESS(MEM_HEAP_CALLER_ADDRESS_LEVEL+1);
...
void print_memheap_free_error_log(void){
   ...
   for(i = 0; i < MAX_MEMHEAP_FREE_ERROR_LOGS ; i++){
     ...
     {
        MSG_HIGH("Memheap_free:thread_id = 0x%x",memheap_free_error_log[i].tcb,
        0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                              ...
                              a multiple free of the same block is detected */
  {
    ...
    uint32 stats_index=theBlock->stats_index;
    memheap_updatestats(heap_ptr,forw_offset,ksubSize,&stats_index);
   ...
   void *caller=0;
   caller=MEM_HEAP_CALLER_ADDRESS(MEM_HEAP_CALLER_ADDRESS_LEVEL);
   mem_info_free_array[ memheap_free_array_cnt ].caller   = caller;
...
)
{
   ...
   if ((size == 0)  || (block_size == 0) || (block_size & block_mask) != 0) return NULL;
   ...
   #if defined FEATURE_MEM_DEBUG
   newBlock->header_debug_info.caller_ptr=MEM_HEAP_CALLER_ADDRESS(MEM_HEAP_CALLER_ADDRESS_LEVEL+1);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (forw_offset < actualSize) {
    ...
    //before changing the forw_offset modify the stats
        ...
        theBlock->stats_index=stats_index;
   ...
   unsigned long                  section_size)
{
   ...
   if(section_prev !=0xFFFF)
   {
      ...
      else
      {
         ...
         pblk = (uint16 *)theBlock;
         ADD_GUARD_BYTES_TO_USED_HEADER(heap_ptr->magic_num_used,pblk);
         newBlock->section_index = section_prev;
   ...
   else if(section_next !=0xFFFF)
   {
      ...
      {
         (void)findNRemoveFreeBlockInBINs(heap_ptr, theBlock, 0);
         memory_end_ptr = (char *)theBlock + theBlock->forw_offset;
         chunks = (unsigned long) ((memory_end_ptr  - memory_start_ptr) / kMinChunkSize);
         mem_init_block_header((mem_block_header_type *) memory_start_ptr, chunks * kMinChunkSize, heap_ptr);
         newBlock->start_flag = kStartBlock;
         newBlock->section_index = section_next;
         memheap_addNodeToBin(heap_ptr, (mem_block_header_type *) memory_start_ptr);
         pblk = (uint16*)(memory_start_ptr);
         ADD_GUARD_BYTES_TO_FREE_HEADER(heap_ptr->magic_num_free,pblk);
         memheap_copy_frd_offset_at_end((mem_block_header_type *) memory_start_ptr, heap_ptr);
   ...
   else
   {
      ...
      newBlock->last_flag = kLastBlock;
      newBlock->start_flag = kStartBlock;
      newBlock->section_index= heap_ptr->number_of_sections;
      heap_ptr->sections[heap_ptr->number_of_sections].last_block = (mem_block_header_type *) memory_start_ptr;
      memheap_addNodeToBin(heap_ptr, (mem_block_header_type *) memory_start_ptr);
