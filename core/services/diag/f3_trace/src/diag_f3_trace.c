...
$Header: //components/rel/core.adsp/2.2/services/diag/f3_trace/src/diag_f3_trace.c#3 $
...
static boolean diag_f3_trace_buffer_init(void)
{
   ...
   memset(err_f3_trace_buffer, 0, DIAG_F3_TRACE_BUFFER_SIZE);
...
void diag_f3_trace_init(void)
{
   ...
   if (diag_f3_trace_control_mask == 0)
   {
...
void err_f3_save_msg(const void* constant_data_ptr, msg_arg_type * args, uint8 nargs, void* msg, uint32 type)
{
   ...
   /* Proceed with saving msg to buffer */
   {
      ...
      memscpy(record_buf,DIAG_F3_TRACE_MAX_MSG_RECORD_SIZE, &record_hdr, sizeof(diag_f3_trace_hdr_type));
      ...
      memscpy(record_buf+record_length, DIAG_F3_TRACE_MAX_MSG_RECORD_SIZE-record_length, 
