...
$Header: //components/rel/core.adsp/2.2/services/diag/DCM/rtos/src/diagpkt.c#10 $
...
static osal_mutex_arg_t mask_tbl_cs;
...
=============================================================================*/
boolean diagpkt_get_delayed_rsp_id_dcm(byte* out_buf_ptr, unsigned long outbuf_max_bytes, unsigned long* pbytes_written)
{
	boolean success = FALSE;

	if( (out_buf_ptr) && (outbuf_max_bytes == sizeof(delayed_rsp_id) ) && (pbytes_written))
	{
		*((uint16*)out_buf_ptr) = diagpkt_next_delayed_rsp_id(delayed_rsp_id);
...
===========================================================================*/
uint32
diagpkt_ctrl_send_mult_pkt_lsm( PACK(void *) ctrl_pkt, 
                             uint32 ctrl_pktlen, 
                             ...
                             boolean * is_sent )
{
  ...
  if( !is_sent)
  {
    MSG_LOW ("diagcomm_ctrl_send_mult_pkt - Sanity check failed", 0, 0, 0);
  ...
  *is_sent = FALSE;
  ...
  if( !ctrl_pkt || (ctrl_pktlen <= 0))
  {
    ...
    {
      MSG_LOW ("diagcomm_ctrl_send_mult_pkt - No pkt to write 0x%x %d", ctrl_pkt, ctrl_pktlen, 0);
    }
    return added_pkt_len;
  }
  do
...
void diagpkt_fwd_cmd_reg ()
{
  ...
  while( fwd_link != NULL )
  {
    if( (fwd_link->tbl_link->tbl_fwd_status == DIAGPKT_TBL_FWD_STATUS_NOT_SENT) )
    {
       ...
       diagpkt_fwd_tbl_index++ )
  {
    ...
    if( (tbl_ptr != NULL) && (diagpkt_table_fwd_status[diagpkt_fwd_tbl_index] == DIAGPKT_TBL_FWD_STATUS_NOT_SENT) )
    {
#endif // FEATURE_DIAG_MASTER_TBL_LIST
      /*-------------------------------------------------------------------------
      ...
      {
      #if defined(FEATURE_DIAG_MASTER_TBL_LIST)
        fwd_link->tbl_link->tbl_fwd_status = DIAGPKT_TBL_FWD_STATUS_QUEUED;
      #else
      ...
      if( is_sent )
      {
        ...
        while( fwd_link != NULL && fwd_link->tbl_link->tbl_fwd_status == DIAGPKT_TBL_FWD_STATUS_QUEUED )
        {
          fwd_link = q_get(&diagpkt_table_fwd_q);
          fwd_link->tbl_link->tbl_fwd_status = DIAGPKT_TBL_FWD_STATUS_SENT;
          free(fwd_link);
          fwd_link = q_check(&diagpkt_table_fwd_q);
        }
      }
  ...
  if( is_sent )
  {
   ...
   {
      local_count = 0;
      if ( q_item->master_tbl_entry[0].client_id != NULL )
      ...
      len_written = diagpkt_ctrl_send_mult_pkt_lsm( (PACK(void *))&reg_pkt, DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_reg_type), 
...
{
	if( osal_init_mutex(&mask_tbl_cs) != OSAL_SUCCESS )
	{
...
diagpkt_delay_alloc(
  ...
  unsigned int length)
{
  PACK(void *) ptr; /* Pointer to return. */

  unsigned int size = length;

  ptr = diagbuf_alloc (code, size, 0, DIAGBUF_ALLOC_REQUEST_DELAY);
...
diagpkt_commit_dcm (int client_handle,PACKED byte * pkt)
{
	...
	if (pkt)
	{
		length = ((diagpkt_lsm_rsp_type *) pkt)->rsp.length;
...
boolean
diagpkt_tbl_reg_dcm(const byte * tbl_ptr, unsigned int count)
{
	unsigned int idx = 0;
	...
	{
		local_tbl_entry->master_tbl_entry[idx].cmd_code = tbl_params[idx].cmd_code;
		local_tbl_entry->master_tbl_entry[idx].subsysid = tbl_params[idx].subsys_id;
		local_tbl_entry->master_tbl_entry[idx].proc_id = tbl_params[idx].proc_id;
		local_tbl_entry->master_tbl_entry[idx].count = count;
		local_tbl_entry->master_tbl_entry[idx].cmd_code_lo = tbl_params[idx].cmd_code_lo;
		local_tbl_entry->master_tbl_entry[idx].cmd_code_hi = tbl_params[idx].cmd_code_hi;
		if(tbl_params[idx].client_id)
		{
		  local_tbl_entry->master_tbl_entry[idx].client_id = tbl_params[idx].client_id;
                  local_tbl_entry->master_tbl_entry[idx].LSM_obj = tbl_params[idx].signal;
...
/* ==========================================================================
  ...
  =============================================================================*/
boolean
diagpkt_tbl_dereg_dcm (uint32 client_id)
{
   //  unsigned int idx = 0, empty_row = 0;
   boolean ret = TRUE;
   ...
   while (q_cur_ptr != NULL)
   {
      ...
      {
         if (q_cur_ptr->master_tbl_entry[0].client_id == client_id)
         {
...
=============================================================================*/
boolean
diagpkt_mask_tbl_reg (uint32 client_id ,qurt_signal_t *mask)
{
...
=============================================================================*/
boolean
diagpkt_mask_tbl_dereg (uint32 client_id)
{
  boolean ret = TRUE;
  ...
  while (q_cur_ptr != NULL)
  {
    if (q_cur_ptr->mask_client.client_id == client_id)
    {
        ...
        diagpkt_common_proc_req = FALSE;
            ...
            for ( k = 0; k < local_count && !found; k++ )
            {
                if ( q_item->master_tbl_entry[k].cmd_code == cmd_code
                     ...
                     && q_item->master_tbl_entry[k].client_id != NULL
	  ...
	  if ( (pkt_len <= DIAG_MAX_RX_PKT_SIZ) )
	  {
            ...
            */
            ...
            else if ( return_sigs & DIAG_CMD_READ_TOUT_SIG )
            {
              ...
              MSG_HIGH("Client stuck in reading the command",0,0,0);
              rsp_pkt = (void *) diagpkt_err_rsp (DIAG_BAD_CMD_F, req_pkt, pkt_len);
              if (rsp_pkt)
                diagpkt_commit (rsp_pkt);
