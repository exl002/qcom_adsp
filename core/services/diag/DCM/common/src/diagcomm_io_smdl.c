...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagcomm_io_smdl.c#3 $
...
#else
	conn->open_params.smdl_params.dsm_mempool_id = DSM_DIAG_SIO_TX_ITEM_POOL;
#endif
  }
...
void diagcomm_io_smdl_send ( diagcomm_io_conn_type * conn )
{
  ...
  //  #ifdef OPTION
  //  ASSERT( total_pkt_length <= sizeof(diag_tx_pkt));
  //  dsm_pullup_tail(&diag_tx_dsm_ptr, &diag_tx_pkt, total_pkt_length);
  //  conn->diag_tx_iovec.buffer = &diag_tx_pkt;
  //#elif
