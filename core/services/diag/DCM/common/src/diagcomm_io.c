...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagcomm_io.c#3 $
...
#if defined(DIAG_MP_MASTER)
      diagcomm_io_conn[i][j].num_presets = 0;
      diagcomm_io_conn[i][j].log_mask_update[DIAG_STREAM_1-1] = NULL;
      diagcomm_io_conn[i][j].log_mask_update[DIAG_STREAM_2-1] = NULL;
      diagcomm_io_ctrl_conn[i][j].num_presets = 0;
      diagcomm_io_ctrl_conn[i][j].log_mask_update[DIAG_STREAM_1-1] = NULL;
      diagcomm_io_ctrl_conn[i][j].log_mask_update[DIAG_STREAM_2-1] = NULL;
#endif
      
#ifdef DIAG_SIO_SUPPORT
      diagcomm_io_conn[i][j].handle.sio_handle = SIO_NO_STREAM_ID;
      diagcomm_io_conn[i][j].open_params.sio_params.params.port_id = SIO_PORT_NULL;
      
      diagcomm_io_ctrl_conn[i][j].handle.sio_handle = SIO_NO_STREAM_ID;
      diagcomm_io_ctrl_conn[i][j].open_params.sio_params.params.port_id = SIO_PORT_NULL;
#elif defined DIAG_SMDL_SUPPORT
      diagcomm_io_conn[i][j].handle.smdl_handle = NULL;
      diagcomm_io_ctrl_conn[i][j].handle.smdl_handle = NULL;
      diagcomm_io_conn[i][j].diag_prev_tx_pending = FALSE;
      diagcomm_io_ctrl_conn[i][j].diag_prev_tx_pending = FALSE;
#endif
    }
  }
    ...
    case DIAGCOMM_IO_TYPE_SIO:
    {
       ...
       */
       ...
       if( conn->handle.sio_handle != SIO_NO_STREAM_ID )
       {
         ...
         else if ( conn->channel_type == DIAGCOMM_IO_DATA ) // there are only two channel types, checking it anyway.
         {
    ...
    case DIAGCOMM_IO_TYPE_SIO:
    {
      if( (DIAGCOMM_PORT_SIO == conn->port_type) && (DIAGCOMM_PORT_1 == conn->port_num) )
      {
        ...
        int return_val_tout = 0, return_val = 0;
                /* Clearing the signal to avoid any corner case */
