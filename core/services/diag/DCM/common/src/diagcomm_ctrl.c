...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagcomm_ctrl.c#6 $
...
//Buffer to copy the entire DSM packet (if chained)
  ...
  {
    MSG_LOW ("diagcomm_process_inbound_ctrl_msg: Recvd NULL DSM", 0, 0, 0);
    return;
  }
...
diagcomm_ctrl_open( diagcomm_port_type port_type, diagcomm_enum_port_type ctrl_channel )
{
  ...
  if( !conn->connected )
  {
    ...
    {
      MSG_LOW ("diagcomm_ctrl_open - Couldn't get stream.", 0, 0, 0);
    }
  }
...
diagcomm_new_dsm( dsm_mempool_id_type pool_id, boolean priority )
{
  ...
  {
    MSG_LOW ("diagcomm_new_dsm - Ran out of DSM items from pool %d", pool_id, 0, 0);
  }
...
{  
 ...
 /* Master Diag communicates to Slave procs over SIO and SMDL */
 if (conn->port_num == DIAGCOMM_PORT_1 || conn->port_num == DIAGCOMM_PORT_2)
    conn->io_type = DIAGCOMM_IO_TYPE_SIO;
    /* ToDo: Based on port number assigned to RIVA, set io_type to smd lite for RIVA */
#elif defined(DIAG_SMDL_SUPPORT)
...
#if defined (FEATURE_DIAG_8K_QDSP6)&& defined (DIAG_MP_MASTER)
  diagcomm_ctrl_open_cb (SIO_PORT_SMD_DIAG_QDSP);
#else
  diagcomm_ctrl_open_cb (SIO_PORT_SMD_DIAG); 
#endif
...
#elif !defined(DIAG_MP_MASTER) && defined (DIAG_MP)
  (void)diagcomm_ctrl_send_mult_pkt( (PACK(void *))&ctrl_msg, DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_feature_type), 
