...
Copyright (c) 2001-2011 by Qualcomm Technologies, Incorporated.
All Rights Reserved.
...
$Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagmm.c#1 $
...
#if !defined (DIAG_QDSP6_APPS_PROC) && !defined (FEATURE_WINCE)
#include "dog.h"
#include "tmc.h"
#endif
...
void diag_ready_start_task (void)
{
  ...
  int return_val = 0;
  osal_thread_wait(&diag_tcb, DIAG_TASK_START_SIG);
  return_val = osal_reset_sigs(&diag_tcb, DIAG_TASK_START_SIG, &return_sigs);
...
void diag_allow_sleep (void)
{
  ...
  return;
#else
  #if (defined SLEEP_DIAG_OKTS_SIG && !defined FEATURE_WINCE)
    osal_sigs_t return_sigs;  
