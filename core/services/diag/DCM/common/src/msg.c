...
$Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/msg.c#4 $
...
msg_init (void)
{
...
#if defined(DIAG_MP_MASTER)
  uint8 stream_index, index, port_num = 0;
#endif
  ...
  diag_debug_record_buf_cs.name = "MUTEX_EVENT_CS";
  return_val = osal_init_mutex(&diag_debug_record_buf_cs);
...
msg_expand_and_send_legacy_pkt (diag_send_desc_type *desc,byte stream_id)
{
  ...
  if (desc)
  {
    ...
    if (desc->state == DIAG_SEND_STATE_START && msg_store)
    {
      ...
      diag_strlcpy ((char *) msg.file, msg_store->const_data_ptr->fname,
                      MSG_FILE_LEN);
      ...
      diag_strlcpy ((char *) msg.fmt, msg_store->const_data_ptr->fmt,
                      MSG_FMT_LEN);
...
msg_set_rt_mask_adv (uint16 ssid, uint32 mask, byte stream_id, uint8 preset_id)
{
  ...
  for (i = 0; !mstr_found && i < MSG_MASK_TBL_CNT; i++)
  {
...
/*-------------------------------------------------------------------------*/
      ...
      req->ssid_start <= req->ssid_end)
  {
    ...
    if (rsp)
    {
      memscpy ((void *) rsp, 4 * ((rsp->ssid_end - rsp->ssid_start)+1), (void *) req, rsp_len);
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if (rsp)
    {
      memscpy ((void *) rsp, 4 * ((rsp->ssid_last - rsp->ssid_first)+1),(void *) req, rsp_len);
...
msg_pkt_set_all_masks_adv (void *req_pkt, word req_len)
{
  ...
  if (req && (req->header.opcode == MSG_EXT_SUBCMD_SET_ALL_RT_MASKS) )
  {
    ...
    {
      if( (req->id.preset_id >= 0) || (req->id.preset_id <= DIAG_MAX_PRESET_ID) )
      {
        send_error = FALSE;
        stream_id = DIAG_STREAM_1;
        preset_id = req->id.preset_id;
...
msg_format_filename (const char *input_filename, char *output_filename)
{
  ...
  diag_strlcpy (output_filename, p_end, sizeof(output_filename));
