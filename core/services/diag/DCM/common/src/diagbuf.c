...
Copyright (c) 1999-2013 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagbuf.c#6 $
...
static uint32 drop_count_delay = 0;

/* Variables to store allocation requests for DIAG Health */
static uint32 alloc_count_log = 0;
static uint32 alloc_count_f3 = 0;
static uint32 alloc_count_delay = 0;
...
/*===========================================================================
FUNCTION DIAGBUF_RESET_DROP_COUNT_DELAY

DESCRIPTION
    This function resets the internal variables drop_count_delay and 
    alloc_count_delay.
===========================================================================*/
...
/*===========================================================================
FUNCTION DIAGBUF_GET_DROP_COUNT_DELAY

DESCRIPTION
    This function returns the internal variable drop_count_delay.
===========================================================================*/
uint32 diagbuf_get_drop_count_delay( void ) {
    return drop_count_delay;
}
/*===========================================================================
FUNCTION DIAGBUF_GET_ALLOC_COUNT_DELAY

DESCRIPTION
    This function returns the internal variable alloc_count_delay.
===========================================================================*/
uint32 diagbuf_get_alloc_count_delay( void ) {
    return alloc_count_delay;
}
...
retry:
  ...
  if (ptr) {
    ...
    switch ( diagbuf_alloc_request ) {
      ...
      case DIAGBUF_ALLOC_REQUEST_MSG:
          ...
          break;
      case DIAGBUF_ALLOC_REQUEST_DELAY:
          if ( alloc_count_delay == MAX_VALUE_UINT32 ) {
              MSG(MSG_SSID_DIAG, MSG_LEGACY_LOW, "Variable alloc_count_delay has reached maximum value.");
          } else {
              alloc_count_delay++;
          }
          break;
      ...
      case DIAGBUF_ALLOC_REQUEST_MSG:
          ...
          break;
      case DIAGBUF_ALLOC_REQUEST_DELAY:
          if ( drop_count_delay == MAX_VALUE_UINT32 ) {
              MSG(MSG_SSID_DIAG, MSG_LEGACY_LOW, "Variable drop_count_delay has reached maximum value.");
          } else {
              drop_count_delay++;
          }
          break;
  ...
  if (ptr) {
    ...
    {
       MSG_HIGH("Possbile Ring buffer corrupt!", 0, 0, 0);
    }
    ...
    diagbuf_set_bit_outbound_pkt_handler(ptr);
  }
...
============================================================================*/
PACK(void *) diagbuf_alloc (uint8 code, uint32 length, int nice, uint8 diagbuf_alloc_request)
{
  PACK(void *) ptr = NULL; /* Pointer to return.        */
...
diagbuf_status_enum_type diagbuf_send_pkt_single_proc(diag_send_desc_type *desc) {
  ...
  if (!(diagcomm_status() && DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_COMM_S)
    ...
    if (outbound != NULL) {
      ...
      diagcomm_send(item_ptr, outbound_used);
    ...
    while (status == DIAGBUF_READY_S && desc->state < DIAG_SEND_STATE_COMPLETE) {
      if (outbound == NULL) {
        ...
        item_ptr = diagcomm_outbound_alloc(&outbound_used, FALSE, desc->priority);
        diagbuf_new_dsm++;
        new_chain = FALSE;
      ...
      if (outbound == NULL) {
        ...
        if (((((int)desc->last - (int)desc->pkt) + outbound_used + hdlc_pad) >= DIAGBUF_SIO_TX_ITEM_SIZ)
            && (TRUE == first_packet) && (!desc->priority)){   
          ...
          item_ptr = diagcomm_outbound_alloc(&outbound_used, FALSE, FALSE);
          diagbuf_new_dsm++;
        ...
        if ((enc.dest >= enc.dest_last) && (desc->pkt > desc->last) && desc->state == DIAG_SEND_STATE_COMPLETE) {
          ...
          while  ((enc.dest >= enc.dest_last) && (desc->state != DIAG_SEND_STATE_COMPLETE)
            ...
            item_ptr = diagcomm_outbound_alloc(&outbound_used, TRUE, desc->priority);
            diagbuf_new_chain++;
            if (item_ptr) {
              /*Indicate if the data in the dsm item will be HDLC encoded or not*/
              if ((DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_HDLC_ENCODE_IN_APPS))\
        ...
        if (((desc->terminate) && (new_chain || dsm_full) && (item_ptr)) || ((desc->priority) && (item_ptr))) {
...
diagbuf_status_enum_type diagbuf_send_pkt_single_proc_ctrl (diag_send_desc_type *desc)
{
  diagbuf_status_enum_type status = DIAGBUF_READY_S;
  unsigned int outbound_used = 0;
  boolean dsm_full = FALSE;
  ...
  if (!(diagcomm_status() && DIAG_CHECK_MASK (diag_cx_state, DIAG_CX_COMM_S)))
  {
    ...
    if (outbound != NULL)
    {
      diagcomm_send_ctrl(item_ptr_ctrl, 0);
      outbound = NULL;
    }
    /* Now discard inbound data by adjusting the descriptor to indicate
       that all data was sent.  Return status that we are ready for more.  */
  ...
  else if (desc == NULL)
  {
    ...
    if (outbound != NULL)
    {
      ...
      diagcomm_send_ctrl (item_ptr_ctrl, outbound_used);
      /* Note: This occurs when diag is about to give up control of the CPU.
         If tx_cnt happens to pass the threshold, the task suspension will
         occur during the next pass. */
      tx_cnt += outbound_used;
  ...
  else
  {
    while (status == DIAGBUF_READY_S && desc->state < DIAG_SEND_STATE_COMPLETE)
    {
      if (outbound == NULL)
      {
        //TODO: Use a constant in diagtune.h, even though it is benign w/ SIO.
         /* Allocate memory from the communications layer */
        item_ptr_ctrl = diagcomm_outbound_alloc_ctrl (&outbound_used, FALSE, desc->priority);   
	   diagbuf_new_dsm++;
	   new_chain_ctrl = FALSE;
 	   if (item_ptr_ctrl)  
        {     
          diag_ctrl_msg_type* ptr = NULL;
          outbound = item_ptr_ctrl->data_ptr;
          ptr = (diag_ctrl_msg_type*)(outbound);
          ptr->cmd_type = DIAG_CTRL_MSG_DIAGDATA;
          ptr->data_len = sizeof(diag_ctrl_msg_diagdata_type);
          ptr->data.ctrl_msg_diagdata.version = DIAG_CTRL_MSG_DIAGDATA_VER;
          ptr->data.ctrl_msg_diagdata.ctrl_data_stream_num = 1;
          
          enc_ctrl.dest = (void *) ((byte*)outbound + DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_diagdata_type));
      ...
      if (outbound == NULL)
      {
        ...
        diag_hdlc_encode (desc, &enc_ctrl);
      ...
      else
      {
		...
		if (((((int)desc->last - (int)desc->pkt) + outbound_used) >= DIAGBUF_SIO_TX_ITEM_SIZ)
            && (TRUE == first_packet_ctrl) && (!desc->priority))
		{   
           ...
           item_ptr_ctrl = diagcomm_outbound_alloc_ctrl (&outbound_used, FALSE, FALSE);
				diagbuf_new_dsm++;
           ...
           if (item_ptr_ctrl) 
           {
             ...
             ptr->data.ctrl_msg_diagdata.version = DIAG_CTRL_MSG_DIAGDATA_VER;
             ptr->data.ctrl_msg_diagdata.ctrl_data_stream_num = 1;
             //outbound = (byte*)((byte*)ptr + DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_diagdata_type));
             enc_ctrl.dest = (void *) ((byte*)outbound + DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_diagdata_type));
             enc_ctrl.dest_last = (void *) ((byte *) outbound + outbound_used - 1 );
           }
           else
      ...
      if (outbound) 
		...
		while  ((enc_ctrl.dest >= enc_ctrl.dest_last) && (desc->state != DIAG_SEND_STATE_COMPLETE)
				...
				item_ptr_ctrl = diagcomm_outbound_alloc_ctrl (&outbound_used, TRUE, desc->priority);
				diagbuf_new_chain++;
				if (item_ptr_ctrl) {
						outbound = item_ptr_ctrl->data_ptr;    
						new_chain_ctrl = TRUE;
		...
		/* If it is a new chain and a terminal packet - send the data OR
		    if it is a priority packet send the data */
		if (((desc->terminate) && (new_chain_ctrl || dsm_full) && (item_ptr_ctrl)) || ((desc->priority) && (item_ptr_ctrl))) {
...
{
  diag_send_desc_type desc_ctrl;
  diagbuf_status_enum_type status = DIAGBUF_READY_S;
  boolean send_NULL = FALSE;

  if(desc && (stream_id & DIAG_STREAM_2)) /* save desc for second stream before values get modified */
  {
     desc_ctrl.pkt = desc->pkt;
     desc_ctrl.last = desc->last;
     desc_ctrl.state = desc->state;
     desc_ctrl.terminate = desc->terminate;
     desc_ctrl.priority = desc->priority;
  }
  else if (NULL == desc) 
  {
     send_NULL = TRUE;
  ...
  {   
     if(send_NULL) 
        status = diagbuf_send_pkt_single_proc_ctrl (NULL);
     else
     {   
        status = diagbuf_send_pkt_single_proc_ctrl (&desc_ctrl);
        /* Update the desc passed in after sending desc_ctrl, in case this gets sent only to second stream */
        desc->pkt = desc_ctrl.pkt;
        desc->last = desc_ctrl.last;
        desc->state = desc_ctrl.state;
        desc->terminate = desc_ctrl.terminate;
        desc->priority = desc_ctrl.priority;
     }
  }
  return status;
} /* diagbuf_send_pkt */
...
/*lint -restore */
  ...
  else if (status == DIAGBUF_READY_S && diagcomm_status())
  {
...

#if defined FEATURE_WINCE
   #define DIAG_TX_SLEEP_THRESHOLD_REG_ENTRY TEXT("DiagTxSleepThresholdBytes")
   #define DIAG_TX_SLEEP_TIME_REG_ENTRY TEXT("DiagTxSleepTimeMS")
#endif /* FEATURE_WINCE */
/*===========================================================================
...
void diagbuf_tx_sleep_parameters_set(void)
{
  ...
  if ((result = RegCreateKeyEx(
       HKEY_LOCAL_MACHINE, WINDIAG_REGISTRY_PATH, 0, NULL, 0, 0, NULL,
        &hKey, &dwVal)) == ERROR_SUCCESS)
   {
     DWORD dwType = 0;
     DWORD dwSize = sizeof(dwVal);
     if ((RegQueryValueEx(hKey, DIAG_TX_SLEEP_THRESHOLD_REG_ENTRY, NULL, &dwType, (LPBYTE)&dwVal, &dwSize)
     != ERROR_SUCCESS) || (dwType != REG_DWORD))
{
       dwVal = DIAG_TX_SLEEP_THRESHOLD_DEFAULT;
       dwType = REG_DWORD;
       dwSize = sizeof(DWORD);
       RegSetValueEx(hKey, DIAG_TX_SLEEP_THRESHOLD_REG_ENTRY, 0, dwType,(const BYTE*)&dwVal, dwSize);
}
     diagbuf_tx_sleep_threshold = dwVal;

     if ((RegQueryValueEx(hKey, DIAG_TX_SLEEP_TIME_REG_ENTRY, NULL, &dwType, (LPBYTE)&dwVal, &dwSize)
     != ERROR_SUCCESS) || (dwType != REG_DWORD))
     {
       dwVal = DIAG_TX_SLEEP_TIME_DEFAULT;
       dwType = REG_DWORD;
       dwSize = sizeof(DWORD);
       RegSetValueEx(hKey, DIAG_TX_SLEEP_TIME_REG_ENTRY, 0, dwType,(const BYTE*)&dwVal, dwSize);
}
     diagbuf_tx_sleep_time = dwVal;
   }
#endif /* FEATURE_WINCE */
