...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagcomm_sio.c#3 $ $DateTime: 2013/10/03 12:10:05 $ $Author: coresvc $
...
#ifdef FEATURE_WINCE
#include "diagportctrl.h"
#include "diag_shared_i.h"
#include "diagondevice.h"
#endif /* FEATURE_WINCE */
...
static uint32 diagcomm_flow_ctrl_count[2] = {0, 0}; /* Diag Health counter - Keeps track of # of times flow ctrl is triggered on SMD path */
...
#ifdef FEATURE_WINCE
static void portctrl_notify(boolean connected, sio_open_type* pparams);
static long queryportsettings(sio_open_type* pparams);
#endif
...
diagcomm_sio_open (void)
{
  ...
  queryportsettings( &(conn->open_params.sio_params.params) );
#endif
  ...
  {    
    ...
    {
      diagpkt_filtered_cmd_set = FALSE;
      MSG_LOW ("diagcomm_sio_open - Couldn't get stream.", 0, 0, 0);
    }
  }
...
#ifdef FEATURE_WINCE
  /* Notify the portctrl APP of the port open status */
  portctrl_notify( conn->connected, &(conn->open_params.sio_params.params) );
#endif
...
#ifdef FEATURE_WINCE
  /* Notify the portctrl APP of the port status */
  portctrl_notify( conn->connected, &(conn->open_params.sio_params.params) );
#endif /* FEATURE_WINCE */
...
diagcomm_outbound_alloc (unsigned int *count, boolean is_chain, boolean priority)
{
  ...
  if (item_ptr)
  {
    ...
    else if (is_chain)
    {
	    ...
	    dsm_item_type *tmp = diagcomm_sio_outbound_item_ptr;
...
diagcomm_outbound_alloc_ctrl (unsigned int *count, boolean is_chain, boolean priority)
{
  ...
  ASSERT (count);
    ...
    {
        MSG_HIGH ("Ran out of (CTRL) SIO Tx Item Pool DSM items", 0, 0, 0);
        *count = 0;
    }
...
===========================================================================*/
void
diagcomm_send (dsm_item_type * ptr, unsigned int length)
{
 ...
 {      
    if (length > 0)
    {   
        tx_params.dsm_ptr = diagcomm_sio_outbound_item_ptr;
        diagcomm_io_transmit( &diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1], &tx_params );
    }
    else
    {
      if (diagcomm_sio_outbound_item_ptr->used > 0) 
       ASSERT(0);
...
===========================================================================*/
void diagcomm_incr_flow_ctrl_count( diagcomm_port_type port_type )
{
...
#ifdef FEATURE_WINCE
void do_diagcomm_sio_open(diag_sio_port_params* pparams)
{
   if(pparams)
   {
      diagcomm_sio_open_cb(pparams->port);
   }
}

void do_diagcomm_sio_close(void)
{
   diagcomm_sio_close_cb();
}

void portctrl_notify(boolean connected, sio_open_type* pparams)
{
   HANDLE hEvt = NULL;
   if(!pparams)
      return;

   hEvt =  CreateEvent(NULL, FALSE, FALSE,DIAG_PORT_CHANGE_SYNC_EVT_NAME);
   if(NULL == hEvt)
   {
      RETAILMSG(1, (TEXT("WINDIAG: portctrl_notify: Failed to create sync event, error = %d"), GetLastError()));
      return;
   }
   
   if(connected)
   {
      if(SIO_PORT_ONDEVICE_DIAG == pparams->port_id)
         SetEventData(hEvt, PORT_DIAG_ONDEVICE);
      else if(SIO_PORT_USB_DIAG == pparams->port_id)
         SetEventData(hEvt, PORT_DIAG_USB);
      else
         SetEventData(hEvt, PORT_DIAG_NULL);
   }
   else
   {
      SetEventData(hEvt, PORT_DIAG_NULL);
...
static LONG queryportsettings(sio_open_type* pparams)
{
   ...
   DWORD size_port = FSIZ(sio_open_type,port_id);
   DWORD port = 0;
   DWORD Type = REG_NONE;
   ...
   {
      RETAILMSG(1, (L"WINDIAG: Failed to open Registry, error = %d", result));
      return result;
   }
  
   //Query registry for port
   if ((result = RegQueryValueEx(hKey, DIAG_SERVICE_WIN_PORT, 0, &Type,(BYTE*)&(port), 
                                 &size_port) == ERROR_SUCCESS))
       
   {
      if(PORT_DIAG_ONDEVICE == port)
         pparams->port_id = SIO_PORT_ONDEVICE_DIAG;
      else if(PORT_DIAG_USB == port)
         pparams->port_id = SIO_PORT_USB_DIAG;
      else
         pparams->port_id = SIO_PORT_NULL;
   }
