...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.2/services/diag/DCM/common/src/diagcomm_io_sio.c#1 $
...
#include "msg.h"              /* For MSG_HIGH(), etc */
#include "osal.h"
...
void diag_dtr_changed_cb( void * cb_data )
{
  ...
  MSG_HIGH ("diag_dtr_changed_cb: port=%d, channel=%d, status=%d", 
            conn->port_type, conn->channel_type, conn->channel_connected);
...
diagcomm_sio_open_cb (sio_port_id_type port_id)
{
  ...
  return_val = osal_set_sigs(&diag_tcb, DIAG_COMM_OPEN_SIG, &return_sigs);
...
diagcomm_ctrl_open_cb( sio_port_id_type port_id )
{
  ...
  return_val = osal_set_sigs( &diag_tcb, DIAG_COMM_OPEN_SMD_SIG, &return_sigs );
  ASSERT(OSAL_SUCCESS == return_val);
