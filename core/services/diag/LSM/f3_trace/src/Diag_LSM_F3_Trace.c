...
$Header: //components/rel/core.adsp/2.2/services/diag/LSM/f3_trace/src/Diag_LSM_F3_Trace.c#1 $
...
static uint8                   diag_f3_trace_wrap_flag;
...
boolean diag_lsm_f3_trace_deinit(void)
{
   ...
   diag_lsm_f3_trace_init_control_mask_config(NULL);
   diag_lsm_f3_trace_init_detail_mask_config(NULL);
   return (TRUE);
}
...
void diag_f3_save_msg(const void *constant_data_ptr, msg_arg_type *args, uint8 nargs, void *msg, uint32 type)
{
   ...
   {
      int i = 0, record_length = 0;
      uint8 record_buf[DIAG_F3_TRACE_MAX_MSG_RECORD_SIZE];
      ...
      /*
       * Create local copy of F3 trace formatted message in 'record_buf'
       */

      /* Set up msg header */
      diag_f3_trace_set_up_msg_header(nargs, savetime, savevars, (char)byte_sized, type, &record_hdr);
      memcpy(record_buf, &record_hdr, sizeof(diag_f3_trace_hdr_type));
      record_length += (int)sizeof(diag_f3_trace_hdr_type);

      /* Save address of the msg_const_type */
      /* ptr size is same for standard and Qshrink messages */
      memcpy(record_buf + record_length,
             &constant_data_ptr, sizeof(void *));
      record_length += (int)sizeof(void *);
      ...
      if (savetime)
      {
         ...
         memcpy(record_buf + record_length, &shortened_ts,
                sizeof(diag_f3_trace_tstamp_type));
      ...
      if (savevars)
      {
         ...
         for (i = 0; i < nargs; i++)
         {
            memcpy(record_buf + record_length, &args[i], (uint32)bytes_to_copy);
            record_length += bytes_to_copy;
         }
      }
      ...
      /* Save record to the f3 trace buffer */
      diag_f3_trace_to_buffer(write_offset, (uint8 *)record_buf, record_length);
   }
}
