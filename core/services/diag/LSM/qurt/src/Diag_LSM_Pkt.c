...
$Header: //components/rel/core.adsp/2.2/services/diag/LSM/qurt/src/Diag_LSM_Pkt.c#1 $
...
diagpkt_user_tbl_init (void)
{
    ...
    if (!initialized)
    {
      qurt_rmutex_init(&diagpkt_user_table_mutex);
...
boolean Diag_LSM_Pkt_DeInit(void)
{
  ...
  qurt_rmutex_lock(&diagpkt_user_table_mutex);
  ...
  for (i = 0; i < DIAGPKT_USER_TBL_SIZE; i++)
  {
   ...
   qurt_rmutex_unlock(&diagpkt_user_table_mutex);
   return ret;
} /* Diag_LSM_Pkt_DeInit */
...
void diagpkt_delay_commit (PACKED void *pkt)
{
    if (pkt)
    {
       ...
       */
       ...
       if (pkt_len > 0 && (diag_qdi_handle >= DIAG_FD_MIN_VALUE))
       {
        
           if(diag_lsm_write_data((byte*)pdiag_data, pkt_len + DIAG_REST_OF_DATA_POS) < (pkt_len + DIAG_REST_OF_DATA_POS) ) 
           {
		      gPkt_commit_fail++;
           }
       }
         ...
         PACKED void *req_pkt, uint16 req_len)
{
  ...
  {
      MSG_HIGH("Diag_LSM:diagpkt_err_rsp:Non-0 request length (%d) and NULL request pointer!",req_len,0,0);
  }
