...
$Header: //components/rel/core.adsp/2.2/services/diag/LSM/qurt/src/Diag_LSM_Log.c#1 $
...
void log_free (PACKED void *ptr)
{
   if(ptr)
   {
      /* Offset to get to the pointer malloced by log_alloc(). */
      ptr = ((byte *) ptr - (LOG_DIAGPKT_OFFSET + DIAG_REST_OF_DATA_POS));
      DiagSvc_Free(ptr, GEN_SVC_ID);
   }
...
)
{
    ...
    /* It is expected that this will be compiled out in a target build */
      /* not so sure this will be compiled out, since we might be
      setting default mask to LOW. Commenting this out for now. */
