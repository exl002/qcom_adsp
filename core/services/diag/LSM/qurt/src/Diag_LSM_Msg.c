...
$Header: //components/rel/core.adsp/2.2/services/diag/LSM/qurt/src/Diag_LSM_Msg.c#3 $
...
static unsigned int gMsg_commit_fail = 0;
   ...
   if(diag_qdi_handle >= DIAG_FD_MIN_VALUE)
    {
       ...
       if (pMsg)
       {
          if(diag_lsm_write_data(pMsg,nLength) < nLength)
	      {
             gMsg_commit_fail++;
          }
		  msg_save_trace((void *)const_blk, NULL, 0, (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_STANDARD);
   ...
   if(diag_qdi_handle >= DIAG_FD_MIN_VALUE)
    {
       ...
       if (pMsg)
       {
          ...
          if(diag_lsm_write_data(pMsg,nLength) < nLength) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
          {
             gMsg_commit_fail++;
          }
		  msg_save_trace((void *)const_blk, NULL, 0, (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_STANDARD);
      ...
      if (pMsg)
       {
          ...
          if( diag_lsm_write_data(pMsg,nLength) < nLength) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
          {
		    ...
		    msg_save_trace((void *)pconst_blk, f3_args, 1, (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_STANDARD);
          DiagSvc_Free(pMsg, GEN_SVC_ID);
       }
	   else
      ...
      if (pMsg)
       {
          ...
          if(diag_lsm_write_data(pMsg,nLength) < nLength) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
          {
		    ...
		    msg_save_trace((void *)pconst_blk, f3_args, 2, (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_STANDARD);
		    DiagSvc_Free(pMsg, GEN_SVC_ID);
       }
	   else
   ...
   if(diag_qdi_handle >= DIAG_FD_MIN_VALUE)
    {
       ...
       if (pMsg)
       {
          ...
          if(diag_lsm_write_data(pMsg,nLength) < nLength) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
          {
		    ...
		    msg_save_trace((void *)pconst_blk, f3_args, 3, (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_STANDARD);
		    DiagSvc_Free(pMsg, GEN_SVC_ID);
       }
	   else
   ...
   if(diag_qdi_handle >= DIAG_FD_MIN_VALUE)
    {
       ...
       if (pMsg)
       {
          ...
          msg_save_trace((const void *)pconst_blk, f3_args, MIN(num_args,MAX_VAR_ARGS), (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_STANDARD);
	       DiagSvc_Free(pMsg, GEN_SVC_ID);
       }
	   else
 			...
 			** hence 1 is added to include NULL. */
 			...
 			/* str is now pointing to the byte after the last valid byte. str - msg
 			 gives the total length required. */
                        if(diag_lsm_write_data( pMsg, (uint32)(str - (char *)pMsg)) < ((uint32)(str - (char *)pMsg)) )/*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
 			{
 			   gMsg_commit_fail++;
 			}
 			DiagSvc_Free(pMsg, GEN_SVC_ID);
...
void qsr_int_msg_send(const msg_qsr_const_type *const_blk, uint32 isHash)
{
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    {
      qshrink_type = DIAG_F3_TRACE_STANDARD;
    }
    ...
    if (pMsg)
    {
      ...
      msg_save_trace((const void *)const_blk, NULL, 0, (void *)(pMsg + DIAG_REST_OF_DATA_POS), qshrink_type);
      DiagSvc_Free(pMsg, GEN_SVC_ID);
    } 
    else
    {
...
void qsr_int_msg_send_1(const msg_qsr_const_type *const_blk, uint32 isHash, uint32 xx_arg1)
{
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    if (pMsg)
    {
      ...
      msg_save_trace((const void *)const_blk, f3_args, 0, (void *)(pMsg + DIAG_REST_OF_DATA_POS), qshrink_type);
...
void qsr_int_msg_send_2(const msg_qsr_const_type *const_blk,  uint32 isHash, uint32 xx_arg1, uint32 xx_arg2)
{
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    if (pMsg)
    {
      ...
      msg_save_trace((const void *)const_blk, f3_args, 2, (void *)(pMsg + DIAG_REST_OF_DATA_POS), qshrink_type);
...
void qsr_int_msg_send_3(const msg_qsr_const_type *const_blk, uint32 isHash, uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3)
{
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    if (pMsg)
    {
      ...
      msg_save_trace((const void *)const_blk, f3_args, 3, (void *)(pMsg + DIAG_REST_OF_DATA_POS), qshrink_type);
   ...
   if(diag_qdi_handle>=DIAG_FD_MIN_VALUE)
    {
       ...
       pMsg = qsr_msg_send_prep (const_blk, num_args, &nLength,TRUE);
       if (pMsg)
       {
          msg_qsr_type *pTemp = (msg_qsr_type*)(pMsg+DIAG_REST_OF_DATA_POS);
          ...
          msg_save_trace((const void *)const_blk, f3_args, MIN(num_args,MAX_VAR_ARGS), (void *)(pMsg+DIAG_REST_OF_DATA_POS), DIAG_F3_TRACE_QSHRINK);
	       DiagSvc_Free(pMsg, GEN_SVC_ID);
       }
	   else
...
msg_v2_send_var(const msg_v2_const_type *const_blk, uint32 num_args, ...)
{
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    if (pMsg)
    {
      ...
      msg_save_trace((const void *)const_blk, f3_args, MIN(num_args, MAX_VAR_ARGS), (void *)(pMsg + DIAG_REST_OF_DATA_POS),  qshrink_type);
 ...
 unsigned long long timestamp, boolean ts_valid)
{
  ...
  if (valid_ssid && (const_blk->desc.ss_mask & rt_mask))
  {
    ...
    alloc_len =            DIAG_REST_OF_DATA_POS + /* For CorePD to recognize that this is an F3 Msg. */
    ...
    if (pMsg)
    {
       ...
       /* Fill in the fact that this is an F3 Message. */
                               ...
                               unsigned int num_args, unsigned int *pLength, uint32 isHash)
{
  ...
  if (valid_ssid && (const_blk->desc.ss_mask & rt_mask))
  {
    ...
    if (pMsg)
    {
      ...
      /* For CorePD to recognize that this is an F3 Msg
         this is independent of whether the msg is Qshrunk or not*/
      diag_data *pdiag_data = (diag_data *)pMsg;
      pdiag_data->diag_data_type = DIAG_DATA_TYPE_F3;
      ...
      {
        msg_qsr_v2_type *pTemp = (msg_qsr_v2_type *)((byte *)pMsg + DIAG_REST_OF_DATA_POS);
        uint32 fmt_pos = 0;
        msg_get_time(&(pTemp->hdr.ts));
#if (MSG_TIME_FORMAT == MSG_WCDMA_TIME_FORMAT)
        pTemp->hdr.ts_type = MSG_TS_TYPE_GW;
        ...
        fmt_pos =  DIAG_REST_OF_DATA_POS + sizeof(msg_hdr_type);
        memcpy((void *)((char *)(pMsg) + fmt_pos), (void *)&(const_blk->desc), sizeof(msg_desc_type));

        /*The arguments are copied in qsr_msg_send* functions*/
        /*Copy the fmt string*/
        fmt_pos = fmt_pos + sizeof(msg_desc_type) + num_args * sizeof(pTemp->args[0]);
        memcpy((void *)((char *)(pMsg) + fmt_pos), (void *)(const_blk->msg_hash + filename_len + 1),  MIN(MSG_MAX_STRLEN, (int)strlen((char *)const_blk->msg_hash + filename_len + 1)));

        /*Copy the null terminator*/
        fmt_pos = fmt_pos + MIN(MSG_MAX_STRLEN, (int)strlen((char *)const_blk->msg_hash + filename_len + 1));
        memcpy((void *)((char *)(pMsg) + fmt_pos), &null_terminator, 1);
        fmt_pos = fmt_pos + 1;

        /*Copy the file name*/
        memcpy((void *)((char *)(pMsg) + fmt_pos), (void *)(const_blk->msg_hash), filename_len);
        fmt_pos = fmt_pos + filename_len;
