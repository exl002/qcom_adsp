...
$Header: //components/rel/core.adsp/2.2/services/time/src/timer_slaves.c#2 $
...
)
{
  ...
  {
    MSG_ERROR("timer_slave_task_num = %d, max value should be less than = %d"
              ,slave_task_num,MAX_SLAVE_TASKS,0 );
    return;
  }
...
)
{
  ...
  switch(timer_slave_num)
  {
    ...
    default:
      MSG_ERROR("Invalid timer task value = %d",timer_slave_num,0,0);
      return;     
  }
...
)
{
  ...
  for(;;)
  {
    ...
    if(sigs & TIMER_SLAVE_RPT_TIMER_SIG)
    {
      ...
      MSG_HIGH("timer_debug: slave1 dog timer fired",0,0,0);
      #endif
      #endif
    }
    ...
    if ( sigs & TIMER_SLAVE_CMD_Q_SIG )
    {
      ...
      while((cmd_ptr = (timer_slave_cmd_type *) q_get(&timer_slaves_cmd_q[TIMER_SLAVE_TASK1])) != NULL)
      {
        ...
        switch (cmd_ptr->call_back_type) 
        {
          ...
          default:
            MSG_HIGH("timer_slave1_debug: command type not supported",0,0,0);
          break;
        }/*end switch*/
        ...
        {
          MSG_ERROR("Processing of callback fn = 0x%x took 0x%x ticks, threshold = 0x%x ticks",
               fn_address_of_timer,
               cb_processing_time,
...
)
{
  ...
  for(;;)
  {
    ...
    if(sigs & TIMER_SLAVE_RPT_TIMER_SIG)
    {
      ...
      #ifdef TIMER_SLAVE_PRINT_HEALTH_MSG
      MSG_HIGH("timer_debug: slave2 dog timer fired",0,0,0);
      #endif
      #endif
    }
    ...
    if ( sigs & TIMER_SLAVE_CMD_Q_SIG )
    {
      ...
      while((cmd_ptr = (timer_slave_cmd_type *) q_get(&timer_slaves_cmd_q[TIMER_SLAVE_TASK2])) != NULL)
      {
        ...
        switch (cmd_ptr->call_back_type) 
        {
          ...
          default:
            MSG_HIGH("timer_slave2_debug: command type not supported",0,0,0);
          break;
        }/*end switch*/
...
)
{
  ...
  for(;;)
  {
    ...
    if(sigs & TIMER_SLAVE_RPT_TIMER_SIG)
    {
      ...
      #ifdef TIMER_SLAVE_PRINT_HEALTH_MSG
      MSG_HIGH("timer_debug: slave3 dog timer fired",0,0,0);
      #endif
      #endif
    }
    ...
    if ( sigs & TIMER_SLAVE_CMD_Q_SIG )
    {
      ...
      while((cmd_ptr = (timer_slave_cmd_type *) q_get(&timer_slaves_cmd_q[TIMER_SLAVE_TASK3])) != NULL)
      {
        ...
        switch (cmd_ptr->call_back_type) 
        {
          ...
          default:
            MSG_HIGH("timer_slave3_debug: command type not supported",0,0,0);
          break;
        }/*end switch*/
...
)
{
  ...
  {
    MSG_ERROR("time_slave_task_num = %d, max value should be less than = %d"
              ,time_slave_task_num,MAX_SLAVE_TASKS,0 );
    return NULL;
  }
