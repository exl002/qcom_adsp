...
$Header: //components/rel/core.adsp/2.2/services/time/src/timer_client.c#8 $ 
...
{
   time_timetick_type first_non_def_tick;
   timer_invoke_get_time_till_first_non_deferrable_timer_64(timer_client_qdi_handle, &first_non_def_tick);
...
void TimerCallbackHandler(void *parameter)
{
   ...
   while(1)
   {
      ...
      /* If the result comes to be negative, dont process */
      if(result < 0)
      {
         MSG_ERROR("timer qdi server returned %d result", result, 0, 0);
         continue;
      }
...
#ifdef FEATURE_TIMER_CLIENT_DEBUG
      cb_processing_started = qurt_timer_get_ticks();
#endif /* FEATURE_TIMER_CLIENT_DEBUG */
    ...
    ERR_FATAL("On adsp, signal cannot be of rex type. Cb type: %d, sigs_obj: 0x%x, sigs: 0x%x", cb_details.cb_type, cb_details.sig_func_addr, cb_details.mask_data1);
...
#ifdef FEATURE_TIMER_CLIENT_DEBUG
      cb_processing_time = qurt_timer_get_ticks() - cb_processing_started;
      ...
      {
         MSG_ERROR("callback fn = 0x%x took %d ticks, process = %d",
