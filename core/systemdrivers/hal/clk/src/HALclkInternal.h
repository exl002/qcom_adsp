...
$Header: //components/rel/core.adsp/2.2/systemdrivers/hal/clk/src/HALclkInternal.h#6 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------- 
09/11/12   dcf     Merged PLL Type (SR, SR2) field to PLL Context typedef
...
#define HAL_CLK_CHIP_SOC_MSM8974    0
#define HAL_CLK_CHIP_SOC_MDM9x25    1
#define HAL_CLK_CHIP_SOC_MSM8x26    2
#define HAL_CLK_CHIP_SOC_APQ8084    3
#define HAL_CLK_CHIP_SOC_MSM8x10    4
#define HAL_CLK_CHIP_SOC_MSM8962    5

#define HAL_CLK_CHIP_FLAG_MSM8974   (1 << HAL_CLK_CHIP_SOC_MSM8974)
#define HAL_CLK_CHIP_FLAG_MDM9x25   (1 << HAL_CLK_CHIP_SOC_MDM9x25)
#define HAL_CLK_CHIP_FLAG_MSM8x26   (1 << HAL_CLK_CHIP_SOC_MSM8x26)
#define HAL_CLK_CHIP_FLAG_MSM8x10   (1 << HAL_CLK_CHIP_SOC_MSM8x10)
#define HAL_CLK_CHIP_FLAG_APQ8084   (1 << HAL_CLK_CHIP_SOC_APQ8084)
#define HAL_CLK_CHIP_FLAG_MSM8962   (1 << HAL_CLK_CHIP_SOC_MSM8962)


#define HAL_CLK_CHIP_FLAG_ALL       0xFFFFFFFF
 ...
 * SlewConfigPLL   - Dynamically re-configure (slew) the source using the same VCO.
 */
typedef struct
{
  ...
  void    (*EnableVote)         ( HAL_clk_SourceDescType *pSource, boolean async );
  void    (*DisableVote)        ( HAL_clk_SourceDescType *pSource);
  boolean (*IsVoteEnabled)      ( HAL_clk_SourceDescType *pSource );
  void    (*SlewConfigPLL)      ( HAL_clk_SourceDescType *pSource, const HAL_clk_PLLConfigType *pmConfig );
  void    (*EnableVoteOverride) ( HAL_clk_SourceDescType *pSource, HAL_clk_ClockDomainDescType *pmClkDomainDesc );
  void    (*DisableVoteOverride)( HAL_clk_SourceDescType *pSource, HAL_clk_ClockDomainDescType *pmClkDomainDesc );
} HAL_clk_SourceControlType;
 ...
 * nChipsetFlag - Supported chipsets for this clock.
 *               
 */
 ...
 * mSPMHWPLLVoter    - HW PLL voting which, if set to TRUE, will ensure that
 *                     the SPMCTL sequence does not override the PLL if it is needed.
 */
 ...
 * SPMHWVoteRegister - Register to set a HW vote to prevent SPMCTL from
 *                     overriding the PLL votes.  Note:  Setting a vote to this register
 *                     will keep the PLL on at power collapse.
 */
typedef struct
{
  uint32                   nAddr;
  ...
  HAL_clk_RegisterMaskType SPMHWVoteRegister;
} HAL_clk_PLLContextType;
...
void HAL_clk_FilterClockDomains ( HAL_clk_ClockDomainDescType** pDomain );
...
HAL_clk_FilterDomain

Description:
  Searches a clock domain and removes clocks that are not supported on the
  current target.
