...
/*============================================================================
  ...
  $Header: //components/rel/core.adsp/2.2/buses/spi/src/driver/DalSpi.c#3 $$DateTime: 2013/10/07 14:16:39 $$Author: coresvc $
...
(DALSYSEventHandle hEvent, void * pInDevCtxt)
{
	...
	do {
		...
		if(pPrevTransaction->spiTransfer.dalSysEvent)
		{
			 ...
			 * function
			 */
			SPI_INFO_LOG_LEVEL1(1, pDevCtxt->DevId,"Notify Transfer(%x) Complete",
							           	pPrevTransaction->u32TransferId);
...
)
{
   ...
   *transferId = pNewTransaction->u32TransferId;
   ...
   SPI_INFO_LOG_LEVEL1(1, pDevCtxt->DevId, "Added TX id %x to Queue",
					             pNewTransaction->u32TransferId);
                                ...
                                SpiTransactionType *pTransac2Schedule)
{
	...
	SPI_INFO_LOG_LEVEL1(1, pDevCtxt->DevId, "Transfer(%x) Queued to HW",
					           	pTransac2Schedule->u32TransferId);
...
DALResult Spi_DeviceInit(SpiClientCtxt *pClientCtxt)
{
  ...
  if(!pDevCtxt->dwWorkLoopCreated)
  {
	  if(DALSYS_SyncCreate(DALSYS_SYNC_ATTR_RESOURCE,
			  ...
			  != DAL_SUCCESS)
	  {
		  SPI_ERROR_LOG_LEVEL0(0, pDevCtxt->DevId, "Spi_DeviceInit:"
                           "Error in synchronize between	client and device");
		  return DAL_ERROR;
	  }
	  ...
	  {
		  SPI_ERROR_LOG_LEVEL0(0, pDevCtxt->DevId, "Spi_DeviceInit:"
                           "Error creating start workloop event");
		  return DAL_ERROR;
	  }
   ...
   {
	    SPI_ERROR_LOG_LEVEL0(0, pDevCtxt->DevId, "Spi_DeviceInit:"
                           "Error in initialization of SPI driver");
	    return DAL_ERROR;
   }
...
DALResult Spi_DeviceDeInit(SpiClientCtxt *pClientCtxt)
{
   ...
   {
	   SPI_INFO_LOG_LEVEL2(0, pDevCtxt->DevId, "DE-INITIALIZED");
  ...
  uint32*  pTransferId, uint32 olen7) 
{
   ...
   SPI_INFO_LOG_LEVEL1(1, pDevCtxt->DevId,"Spi_StartTransfer:"
                       "SPI data transfer request status to DAL1 queue %x"
					             , transfStatus);
