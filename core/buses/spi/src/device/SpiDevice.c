...
$Header: //components/rel/core.adsp/2.2/buses/spi/src/device/SpiDevice.c#3 $

When     Who    What, where, why
-------- ---    -----------------------------------------------------------
11/05/13 ms     Klockwork Error fixed
...
)
{
   ...
   if (NULL == phDev) 
   {
      IBUSLOG_LOG_LEVEL_1(0,"SpiDevice_Init: Null pointer passed \n");
   ...
   if (SPIDEVICE_RES_SUCCESS != res) 
   {
      IBUSLOG_LOG_LEVEL_1(0,"SpiDevice_Init: Malloc failure\n");
      return res;
   }
...
)
{
   ...
   if (NULL == hDev) 
   {
      IBUSLOG_LOG_LEVEL_1(0,"SpiDevice_DeInit: Null pointer passed \n");
      return SPIDEVICE_ERR_INVALID_DEV_HANDLE;
   }
...
)
{
   ...
   if (pDev->platInfo.bInterruptBased) /* Interrupt based wait.          */
   {
      ...
      if (IBUSOS_RESULT_ERROR_TIMEOUT == osRes) {
         IBUSLOG_LOG_LEVEL_1(0,"QUP_WaitForCompletion:Timeout Failure\n");
         bTimeout = TRUE;
      }
...
) 
{
   ...
   {
      IBUSLOG_LOG_LEVEL_1(1,"Failed to set frequency matching to slave spec");
...
) 
{
   ...
   if(pDev->bamObj)
   {
      if(TRUE == pDev->bamObj->useCmdDescs)
      {
...
)
{
   ...
   do
   {
	  ...
	  if(pDev->bamObj)
	  {   
         if (FALSE == pDev->bamObj->useCmdDescs)
         {
            res = SpiDevice_SetRunState((uint8*)pDev->platInfo.qupVirtBlockAddr,
...
)
{
   ...
   *puNumCompleted = 0;
   ...
   do{
         ...
         {
            eot = (uNumTransferVecs == 1)?TRUE:FALSE;
         }
   ...
   }while((--uNumTransferVecs) && (++pTransfer) && (SPIDEVICE_RES_SUCCESS == res));
