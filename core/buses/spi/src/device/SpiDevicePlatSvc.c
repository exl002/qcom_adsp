...
$Header: //components/rel/core.adsp/2.2/buses/spi/src/device/SpiDevicePlatSvc.c#4 $
...
)
{
   ...
   {
      IBUSLOG_LOG_LEVEL_1(0,"IBUSPLATCFG_AddDevCfg: Malloc failure\n");
      return res;
   }
...
)
{
   ...
   if (IBUSOS_RESULT_OK != res) {
      IBUSLOG_LOG_LEVEL_1(0,"IBUSPLATCFG_RemoveDevCfg: Free failure\n");
      return res;
   }
...
)
{
   ...
   if (NULL == pTlmmHandle)
   {
      IBUSLOG_LOG_LEVEL_1(0,"spi pTlmmHandle not initialised");
      return DAL_ERROR;
   }
   ...
   if( DAL_SUCCESS == DALSYS_GetDALPropertyHandle(pDev->uOsDevId, hProp))
   {
      ...
      {
         IBUSLOG_LOG_LEVEL_1(0,"spi fail to get clk gpio number");
         return DAL_ERROR;
      }
      ...
      {
         IBUSLOG_LOG_LEVEL_1(0,"No GPIO assigned for spi cs");
      ...
      {
         IBUSLOG_LOG_LEVEL_1(0,"spi fail to get mosi gpio number");
         return DAL_ERROR;
      }
      ...
      {
         IBUSLOG_LOG_LEVEL_1(0,"spi fail to get miso gpio number");
         return DAL_ERROR;
      }
   }
   else
   {
      IBUSLOG_LOG_LEVEL_1(0,"spi ConfigGPIO fail to get property handle");
      return DAL_ERROR;
   }
   ...
   {
      IBUSLOG_LOG_LEVEL_1(0,"spi ConfigGPIO fail on clkSig");
      return DAL_ERROR;
   }
   ...
   if(bCsGPIOUsed)
   {
      ...
      if (csSig.dwLen > 0)
      {
         ...
         {
            IBUSLOG_LOG_LEVEL_1(0,"spi ConfigGPIO fail on csSig");
            return DAL_ERROR;
         }
   ...
   if(DAL_SUCCESS != DalTlmm_ConfigGpio(pTlmmHandle, 
                           ...
                           DAL_TLMM_GPIO_ENABLE))
   {
      IBUSLOG_LOG_LEVEL_1(0,"spi ConfigGPIO fail on mosiSig");
      return DAL_ERROR;
   }
   ...
   if(DAL_SUCCESS != DalTlmm_ConfigGpio(pTlmmHandle, 
                          ...
                          DAL_TLMM_GPIO_ENABLE))
   {
      IBUSLOG_LOG_LEVEL_1(0,"spi ConfigGPIO fail on misoSig");
      return DAL_ERROR;
   }
...
static ISpiDevice_Result IBUSPLATCFG_AddPNOCVote(IBUSPLATCFG_DevTargetCfgType *pDev)
{
...
static ISpiDevice_Result IBUSPLATCFG_RemovePNOCVote(IBUSPLATCFG_DevTargetCfgType *pDev)
{ 
   if(pDev->pNpaHandle == NULL)
   {
...
/** @brief Returns whether command descriptors can be used.

    @param[in] hPlat platform device handle.

    @return              boolean.
  */
boolean IBUSPLATCFG_UseCmdDescriptors
(
   IBUSPLATCFG_HANDLE hPlat
)
{
