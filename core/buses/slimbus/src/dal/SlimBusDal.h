...
$Header: //components/rel/core.adsp/2.2/buses/slimbus/src/dal/SlimBusDal.h#4 $
 ...
 * @return SB_SUCCESS on success, error code on error
 */
SLIMBUS_INLINE SBResult SlimBus_EventTriggerEx(SlimBusEventHandle hEvent, uint32 uUserData)
{
  return DALSYS_EventCtrlEx(hEvent, DALSYS_EVENT_CTRL_TRIGGER, uUserData, NULL, 0);
}
 ...
 * @return SB_SUCCESS on success, error code on error
 */
SLIMBUS_INLINE SBResult SlimBus_CreateTimoutEvent(SlimBusEventHandle *phEvent, SlimBusEventObj *pEventObj)
{
  return DALSYS_EventCreate(DALSYS_EVENT_ATTR_TIMEOUT_EVENT|DALSYS_EVENT_ATTR_NORMAL, phEvent, pEventObj);
}
 ...
 * @param[in] pDevCtxt  Pointer to the Slimbus device 
 */
SLIMBUS_INLINE void SlimBus_InterruptAcquireLock(SlimBusDevCtxt *pDevCtxt)
{
  /* do nothing, use device lock instead */
}
 ...
 * @param[in] pDevCtxt  Pointer to the Slimbus device
 */
SLIMBUS_INLINE void SlimBus_InterruptReleaseLock(SlimBusDevCtxt *pDevCtxt)
{
  /* do nothing, use device lock instead */
}
 ...
 * @return SB_SUCCESS on success, error code on error
 */
SLIMBUS_INLINE SBResult SlimBus_PhysMemInfo(SlimBusMemHandle hMem, uint32 *puVirtAddr, uint32 *puPhysAddr)
{
