 ...
 * 
 * $Header: //components/rel/core.adsp/2.2/buses/icb/src/common/axicfgint.c#3 $ 
...
static void _AxiCfgInt_SetBIMCMasterDefaults( icb_master_def_type *pMaster )
{
  ...
  for( port = 0; port < pMaster->uNumPorts; port++ )
  {
    ...
    /* Default clock gating settings. */
    HAL_bimc_SetMasterClockGating( pInfo, pMaster->pPortIds[port], &pDefs->bimcClockGate );
      ...
      switch( pSlave->pBus->busType )
      {
        case ICB_BUS_BIMC:
        {
          ...
          for( port = 0; port < pSlave->uNumPorts; port++ )
          {
            ...
            HAL_bimc_SetSlaveClockGating(  pInfo, pSlave->pPortIds[port], &pDefs->bimcClockGate );
...
bool AxiCfgInt_Init( void )
{
  ...
  if( AXICFGINT_STATE_UNINITIALIZED == state && AxiCfgOs_Init() )
  {
    ...
    for( i=0; i<SocInfo.uBusCount; i++ )
    {
      ...
      {
        case ICB_BUS_BIMC:
          ((HAL_bimc_InfoType *)pCurrBus->pInfo)->uBaseAddr = AxiCfgOs_Mmap( pCurrBus->pszBaseName, pCurrBus->uBaseAddr, pCurrBus->uRegionSize );
          if ( 0 != ((HAL_bimc_InfoType *)pCurrBus->pInfo)->uBaseAddr )
          {
            AxiCfgOs_MutexLock( ); 
          ...
          break;
        case ICB_BUS_NOC:
          ((HAL_noc_InfoType*)pCurrBus->pInfo)->uBaseAddr = AxiCfgOs_Mmap( pCurrBus->pszBaseName, pCurrBus->uBaseAddr, pCurrBus->uRegionSize );
          if( 0 != ((HAL_noc_InfoType*)pCurrBus->pInfo)->uBaseAddr )
          {
            AxiCfgOs_MutexLock( ); 
...
)
{
  ...
  if( ICB_BUS_NOC == SearchInfo.pCurrMaster->pBus->busType )
  {
    ...
    if( !AxiCfgInt_IsRemoteMaster( &SearchInfo ) &&
        SearchInfo.pCurrMaster->pDefaults )
    {
