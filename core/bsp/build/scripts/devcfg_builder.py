...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.adsp/2.2/bsp/build/scripts/devcfg_builder.py#5 $
#  $DateTime: 2013/07/10 18:33:19 $
#  $Author: coresvc $
#  $Change: 4074731 $
...
def extract_c_struct_variable_from_xml(xml_file,env):
    unique_struct_list = []
    rootenv = env.get('IMAGE_ENV')
    if rootenv.GetUsesFlag('USES_DEVCFG_MULTIPLE_CONFIG') is False:
...
def devcfg_xml_builder(target, source, env):
    ...
    master_xml = devcfg_create_master_xml_file(rootenv, target_full , env['CONFIG'], env)
    
    return
    
    
def devcfg_create_master_xml_file(env, filepathname, env_config_list, dalenv):
    """
