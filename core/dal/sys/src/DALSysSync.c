...
/* highest priority in qurt */
#define DISABLE_PREEMPTION_PRIORITY DAL_PRIO_RESERVED 
#endif

#ifdef DAL_USE_PIMUTEX
...
static void sync_disable_preemption(_DALSYSSyncObj *pSyncObj)
{
   if(!pSyncObj->orig_prio) /* is this a nested call to disable preemption ? */
   {
      ...
      qurt_thread_set_priority(qurt_id, DISABLE_PREEMPTION_PRIORITY);
   }
