...
$Header: //components/rel/core.adsp/2.2/power/npa/src/framework/npa_graph.c#2 $
============================================================================*/
...
/**
 * <!-- npa_resource_unconditional_first_drv_fcn -->
 *
 * @brief Set as the resource's active driver fcn during npa_create_node if 
 * the resource has the NPA_RESOURCE_DRIVER_UNCONDITIONAL_FIRST attribute. 
 */
static npa_resource_state npa_resource_unconditional_first_drv_fcn( 
                                                   npa_resource      *resource,
                                                   npa_client_handle  client,
                                                   npa_resource_state state )
{
  resource->definition->attributes &= ~NPA_RESOURCE_DRIVER_UNCONDITIONAL;
  resource->_internal->active_driver_fcn = resource->node->driver_fcn;
  return resource->_internal->active_driver_fcn( resource, client, state );
}
...
static npa_resource_internal_data npa_internal_data_nodata = { NULL };
                             ...
                             npa_callback_event  *callback )
{
        ...
        resource_def = &definition->resources[++i] )
  {
    ...
    /* Initialize the active driver function from definition. Doing it here 
     * allows the resource to modify the driver function within 
     * CLIENT_INITIALIZE, should it choose/need to */
    resource->_internal->active_driver_fcn = resource->node->driver_fcn;

    /* Check if the resource has the attribute 
     * NPA_RESOURCE_DRIVER_UNCONDITIONAL_FIRST.  If it does, set
     * NPA_RESOURCE_DRIVER_UNCONDITIONAL and driver function that will call the
     * original driver and unset NPA_RESOURCE_DRIVER_UNCONDITIONAL.
     */
    if ( npa_resource_has_attribute( resource, 
                                     NPA_RESOURCE_DRIVER_UNCONDITIONAL_FIRST ) )
    {
      resource->definition->attributes |= NPA_RESOURCE_DRIVER_UNCONDITIONAL;
      resource->_internal->active_driver_fcn = 
        npa_resource_unconditional_first_drv_fcn;
    }
  }
