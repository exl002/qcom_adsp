...
$Header: //components/rel/core.adsp/2.2/power/npa/src/framework/npa_dump.c#2 $
============================================================================*/
#include <stdio.h>
                                      ...
                                      npa_resource *resource )
{
  ...
  if(NULL == internal_data)
  {
    ULOG_RT_PRINTF_3(log,
                     ...
                     "(event_lock: %08X) "
                     "(_internal: %08X) ",
                     (unsigned int)resource->node_lock,
                     (unsigned int)resource->event_lock,
                     (unsigned int)internal_data );
  }
  else
  {
    ...
    ULogFront_RealTimePrintf(log, 10,
                             ...
                             "(count: %llu) "
                             "(total: %llu))",
                             ULOG64_LOWWORD( duration_data->min ),
                             ULOG64_DATA( duration_data->min_time ),
                             ULOG64_LOWWORD( duration_data->max ),
                             ULOG64_DATA( duration_data->max_time ),
