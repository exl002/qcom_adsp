...
$Header: //components/rel/core.adsp/2.2/power/utils/src/CoreWorkQueue.c#2 $
============================================================================*/
#include "DALSys.h"
 ...
 * @param where:      HEAD or TAIL of the queue
                                       ...
                                       unsigned int no_dups )
{
  ...
  if ( no_dups )
  {
    /* The queue is locked and if this element matches the head, tail,
       or has it's pNext pointer set, we can deduce it's currently in the 
...
DALResult Core_WorkQueuePut( CoreWorkContainerHandle  hContainer,
                             void                    *pData)
{
  return Core_WorkQueueInsert( hContainer, pData, TAIL, 0 );
}
...
DALResult Core_WorkQueuePutAtHead( CoreWorkContainerHandle  hContainer,
                                   void                    *pData )
{
  return Core_WorkQueueInsert( hContainer, pData, HEAD, 0 );
}
...
DALResult Core_WorkQueuePutNoDups( CoreWorkContainerHandle  hContainer,
                                   void                    *pData )
{
  return Core_WorkQueueInsert( hContainer, pData, TAIL, 1 );
}
...
DALResult Core_WorkQueuePutAtHeadNoDups( CoreWorkContainerHandle  hContainer,
                                         void                    *pData )
{
  return Core_WorkQueueInsert( hContainer, pData, HEAD, 1 );
}
