...
$Header: //components/rel/core.adsp/2.2/power/ulog/src/ULogCore.c#2 $
============================================================================*/
...
/** @brief The global pointer to the linked list of ULogs */
...
ULogResult ULogCore_HeaderSet(ULogHandle h, char * headerText)
{
...
#endif
  {
    ...
    DALSYS_Malloc(strlen(headerText)+1, (void **)&log->header);
    if (log->header != NULL)
    {
      core_strlcpy(log->header, headerText, strlen(headerText)+1);
    }
    else
    {
 ...
 * @param secondMsgContent : The remaing data to put into the log. 
                           ...
                           ULOG_INTERFACE_TYPE interfaceType )
{
  ...
  //check the write and readWriter for available space, free space if needd.
...
static ULogHandle ULogCore_TeeLinkRemove(ULOG_TEE_TYPE *tee, ULOG_TYPE *log)
{
  ...
  else
  {
    ...
    {
      if(link1->log == log)
      { // Remove the first occurance - there should be only one.
