...
$Header: //components/rel/core.adsp/2.2/power/ulog/src/ULogParsers.c#2 $ 
============================================================================*/
 ...
 * @param msg : The unformated binary msg data
 ...
 *
 * When a format letter like x or d is found the the format string is done. 
 *
                                   ...
                                   unsigned int bytesToRead )
{
  ...
  {
    memcpy(dest, parse_data->msgpart1, bytesToRead);
  ...
  {
    memcpy(dest, parse_data->msgpart2, bytesToRead);
                                    ...
                                    ULOG_READ_TO_ASCII_DATA* mpm_parse_data )
{
  ...
  {  
     ...
     // move forward to parse the next part of the mesage and update pointers
                                       ...
                                       ULOG_READ_TO_ASCII_DATA* mpm )
{
  ...
  // Intialize the pointer used to move around format the string. 
  fmtPtr = fmtString;
        ...
        { 
          ...
          if (parse_data->part1_size >= 4)
          {
            ...
            else 
            {
              ...
              // Read in the pointer refered to us by the % format character.
              UlogReadNextMsgPart((unsigned char*)&localWord, parse_data, 4);
                                        ...
                                        ULOG_READ_TO_ASCII_DATA* mpm )
{
  ...
  switch (msgFormat)
  {
    ...
    case ULOG_REALTIME_SUBTYPE_RESERVED_FOR_RAW:   // An Raw 1 byte format mesage
      ULFE_RawParse(parse_data, mpm, 1);
      break;

    case ULOG_SUBTYPE_RAW16:
      ULFE_RawParse(parse_data, mpm, 2); // An Raw 2 byte format mesage
      break;

    case ULOG_SUBTYPE_RAW32:
      ULFE_RawParse(parse_data, mpm, 4); // An Raw 4 byte format mesage
      break;
