...
$Header: //components/rel/core.adsp/2.2/power/ulog/src/ULogInlines.h#2 $
============================================================================*/
...
static ULOG_INLINE void ULogInternal_WriteData(ULOG_TYPE * log, uint32 msgCount, const char * msgContent)
{
  ...
  {
    memcpy(&(log->buffer[log->writerCore->write & log->bufSizeMask]), msgContent, msgCount);
  }
  else
  {
    copy1Count = bytesToEnd;
    copy2Count = msgCount - bytesToEnd;
    memcpy(&log->buffer[log->writerCore->write & log->bufSizeMask], msgContent, copy1Count);
    memcpy(&log->buffer[0], &msgContent[copy1Count], copy2Count);
  }
