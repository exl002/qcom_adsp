...
$Header: //components/rel/core.adsp/2.2/power/ulog/src/ULogDiag.c#2 $
============================================================================*/
...
static ULogHandle GetExternalHandle(ULogHandle handle)
{
  ...
  if (ulogDiagExtHeadPtr == NULL)   //no preexising list of handles, start one
  {
 ...
 * <!-- GetInternalHandle -->
 * 
 * @brief Given an external handle, find the coresponding internal handle and return it.
 *
 ...
 * 
 * When the PC based ULogDiagPluginLauncher.exe is run, the a plugin with a 
 ...
 * @param new_plugin_id : A uint32 id that the plugin will be registed with.  
 *
...
ULogResult ULogDiagAddPlugin(int(*new_pluginfcn)(uint32), uint32 new_plugin_id){
  ...
  if (ulogDiagPluginHeadPtr == NULL)   //no preexising list of handles, start one
  {
      ...
      switch (command_code)
      {
        ...
        case ULOGDIAG_CONNECT:
          ...
          if (tempLog!=NULL){
            ...
            {
              // This should be almost impossible unless a malloc getting an extral handle fails. 
