...
//                             Edit History
//
// $Header: //components/rel/core.adsp/2.2/debugtrace/stm/dal/inc/DDISTMTrace.h#1 $
//
...
static __asm void incr2_word_write(STMTracePortType uPort, uint32 * data)
{
   LDMIA r1!,{r2-r3}
   STMIA r0!, {r2-r3}
   BX lr
...
static __inline void DalSTMTrace_Data64(DalDeviceHandle * _h, STMTracePortType uPort,
                                        STMTraceDataType uType, uint32 * uData)
{
   uPort = ((uPort)|(0x98&(~uType)));
   incr2_word_write(uPort, uData);
}

static __asm void incr4_word_write(STMTracePortType uPort, uint32 * data)
{
   PUSH {r4-r5}
   LDMIA r1!,{r2-r5}
   STMIA r0!, {r2-r5}
...
static __inline void DalSTMTrace_Data128(DalDeviceHandle * _h, STMTracePortType uPort,
                                         STMTraceDataType uType, uint32 * uData)
{
   uPort = ((uPort)|(0x98&(~uType)));
   incr4_word_write(uPort, uData);
}

#endif //(DAL_CONFIG_ARCH & DAL_CONFIG_ARCH_ARM)

static __inline void DalSTMTrace_Data32(DalDeviceHandle * _h, STMTracePortType uPort, STMTraceDataType uType, uint32 uData)
{
   *(volatile uint32 *)((uPort)|(0x98&(~uType)))=uData;
}

static __inline void DalSTMTrace_Data16(DalDeviceHandle * _h, STMTracePortType uPort, STMTraceDataType uType, uint16 uData)
{
   *(volatile uint16 *)((uPort)|(0x98&(~uType)))=uData;
}

static __inline void DalSTMTrace_Data8(DalDeviceHandle * _h, STMTracePortType uPort, STMTraceDataType uType, uint8 uData)
{
   *(volatile uint8 *)((uPort)|(0x98&(~uType)))=uData;
}

static __inline void DalSTMTrace_Flag(DalDeviceHandle * _h, STMTracePortType uPort, STMTraceFlagType uType)
{
   *(volatile uint32 *)((uPort)|(0xE8&(~uType)))=0;
}

static __inline void DalSTMTrace_Trigger(DalDeviceHandle * _h, STMTracePortType uPort, STMTraceTriggerType uType)
{
