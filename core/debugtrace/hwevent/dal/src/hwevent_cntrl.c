...
/*===========================================================================
  ...
  $Header: //components/rel/core.adsp/2.2/debugtrace/hwevent/dal/src/hwevent_cntrl.c#1 $
...
int qdss_hwevent_configure_diag_handler(qdss_hwevent_configure_diag_req *pReq,
                          ...
                          int rsp_len)
{
   uint8 nErr = QDSS_HWEVENT_UNKNOWN_ERR;
...
*/
   if(hHWEvent==NULL)
   {
      if(DAL_SUCCESS!=DAL_HWEventDeviceAttach("DALDEVICEID_HWEVENT",&hHWEvent))
      {
         pRsp->result = nErr;
         return nErr;
      }
