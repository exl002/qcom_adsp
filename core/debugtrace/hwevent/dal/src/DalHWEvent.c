...
$Header: //components/rel/core.adsp/2.2/debugtrace/hwevent/dal/src/DalHWEvent.c#1 $
=============================================================================*/
...
static __inline boolean HWEvent_isBlockedAddress(uint32 phys_addr,HWEventDevCtxt *pDevCtxt)
{
   HWEventAddrRange *pAddrRangeTable;
   uint32 i;
   i=0;
...
/*register peek-poke functionality needs to be guarded by a security fuse*/
   ...
   if(pDevCtxt==NULL)
     return DAL_ERROR;
   if (HWEvent_isBlockedAddress(phys_addr,pDevCtxt))
   {
...
/*Silently drop writes to blocked address regions without flagging an error*/  
      if (!HWEvent_isBlockedAddress(pDevCtxt->pConfigTable[i].reg_phys_addr,pDevCtxt))
      {
