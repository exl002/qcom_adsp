...
/*=============================================================================
Edit History
$Header: //components/rel/core.adsp/2.2/debugtools/utils/src/tms_utils_fmt.c#1 $
...
static int8_t* tms_utils_fmt_put_chr(int8_t* out_p, const size_t out_buf_sz, const int8_t chr)
{
...
static int8_t* tms_utils_fmt_put_chr_buf(int8_t* out_p, const size_t out_buf_sz, int8_t const* in_p)
{
   size_t sz = out_buf_sz;
...
static int32_t tms_utils_fmt_put(int8_t* out_buf_p, const size_t out_buf_sz, char const* fmt_str, va_list argv)
{
   ...
   if (out_buf_p && out_buf_sz && fmt_str)
   {
      ...
      do
      {
         int8_t chr_num[ULONG_BITS + 1];
         int8_t fmt_chr, fmt_code;
         ...
         switch (fmt_code)
         {
            ...
            {
               out_p = tms_utils_fmt_put_chr(out_p, out_buf_sz - (out_p - out_buf_p), (int8_t)va_arg(argv, int));
               continue;
            }

            case ASCII_CAP_S:                                                          // next field is template, %s
            {
               int8_t *chrs_p = va_arg(argv, int8_t*);

               // determine length of passed parameter template

               for (fmt_fill.ul = 0; ASCII_NIL != *(chrs_p + fmt_fill.ul); fmt_fill.ul++)
                  /*NULL*/;
         ...
         if ((ASCII_D == fmt_code || ASCII_CAP_D == fmt_code) && 0 > (typeof(field_arg.l))field_arg.ul)
         {
            field_flag.ul |= FIELD_FLAG_SIGN;

            field_arg.ul = 0 - (typeof(field_arg.l))field_arg.ul;                                            // 2's compliment obtains absolute value
         }
         ...
         do
         {
            ...
            fmt_code = (int8_t)(field_arg.ul % field_radix.ul);
...
int32_t tms_utils_fmt(int8_t* out_buf_p, const size_t out_buf_sz, char const* fmt_str, ...)
{
   int32_t rc = 0;
