...
/* $Header: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/scm/framework/src/sns_scm_ext.c#1 $ */
 ...
 * -------------------------------------------------------------------------*/
extern qmi_client_type sns_scm_smgr_user_handle;
extern qmi_client_type sns_scm_sam_user_handle;
#ifndef ADSP_STANDALONE
extern qmi_client_type sns_scm_reg_user_handle;
   ...
   uint32_t regItemId)
{
   ...
   err = sns_scm_qmi_client_send_msg_async (sns_scm_reg_user_handle, msg_id,
                                    reqMsgPtr, reqSize,
                                    respMsgPtr, respSize,
...
{
   qmi_client_error_type               err;
   sns_smgr_sensor_status_req_msg_v01  *msgReqP;
   sns_smgr_sensor_status_resp_msg_v01 *msgRespP;
   qmi_txn_handle                      txn_handle;

   msgReqP = (sns_smgr_sensor_status_req_msg_v01 *)
             SNS_OS_MALLOC(SNS_SCM_DBG_MOD, sizeof(sns_smgr_sensor_status_req_msg_v01));
   SNS_ASSERT(msgReqP != NULL);

   msgRespP = (sns_smgr_sensor_status_resp_msg_v01 *)
              SNS_OS_MALLOC(SNS_SCM_DBG_MOD, sizeof(sns_smgr_sensor_status_resp_msg_v01));
   SNS_ASSERT(msgRespP != NULL);

   msgReqP->SensorID       = sensorId;
   msgReqP->Action         = SNS_SMGR_SENSOR_STATUS_ADD_V01;
   msgReqP->ReqDataTypeNum = 1;

   err = sns_scm_qmi_client_send_msg_async (sns_scm_smgr_user_handle, SNS_SMGR_SENSOR_STATUS_REQ_V01,
                                    msgReqP, sizeof(sns_smgr_sensor_status_req_msg_v01),
                                    msgRespP, sizeof(sns_smgr_sensor_status_resp_msg_v01),
                                    sns_scm_smgr_resp_cb, NULL, &txn_handle);
   ...
   void* qmagCalAlgoDBConfig)
{
   ...
   err = sns_scm_qmi_client_send_msg_async (sns_scm_reg_user_handle, SNS_REG_GROUP_WRITE_REQ_V02,
...
static sns_err_code_e sns_scm_send_smgr_version_req( void )
{
   ...
   err = sns_scm_qmi_client_send_msg_async (sns_scm_smgr_user_handle, SNS_SMGR_VERSION_REQ_V01,
                                    NULL, 0, // no payload
...
static sns_err_code_e sns_scm_send_registry_version_req( void )
{
   ...
   err = sns_scm_qmi_client_send_msg_async (sns_scm_reg_user_handle, SNS_REG_VERSION_REQ_V02,
                                    NULL, 0, // no payload
                                    msgRespP, sizeof(sns_common_version_resp_msg_v01),
                                    sns_scm_reg_resp_cb, NULL, &txn_handle);
#endif /* ADSP_STANDALONE */
