 ...
 * Size is at least the greater of - 
 * For all algos, 
 *    - number of dependent algos + additional entry for time service.  
 * For SAM framework
 *    - PM, TIME, SMGR, SMGR_INTERNAL, REG and MODEM_SCN
 * As of today, minimum size must be 6. */
#define SNS_SAM_SVC_ARR_LEN SNS_SAM_MAX_ALGO_DEPS + 3
...
/*=========================================================================
  FUNCTION: sns_sam_mr_remove_all_inds
  =========================================================================*/
/*!
@brief
  Sweep through the SAM message queue and remove and free all Indications
*/
/*=======================================================================*/
static void 
sns_sam_mr_remove_all_inds( void )
{
  sns_sam_q_item_s *q_item = NULL, *next_item = NULL;
  void *msg = NULL;
  sns_smr_header_s smr_header;

  q_item = sns_q_check( &sns_sam_msg_queue );
  ...
  while( q_item != NULL )
  {
    ...
    /* Remove all indications that came from SMGR, from the SAM queue */
    if ( ( QMI_IDL_INDICATION == smr_header.msg_type ) && ( SNS_MODULE_DSPS_SMGR == smr_header.src_module ) )
    {
      next_item = sns_q_next( &sns_sam_msg_queue, &q_item->q_link );
      sns_q_delete( &q_item->q_link );
      sns_smr_msg_free( q_item->msg );
      SNS_OS_FREE( q_item );
      q_item = next_item;
    }
    else
    {
      q_item = sns_q_next( &sns_sam_msg_queue, &q_item->q_link );
    }
  }
}
    ...
    void *ind_buf, unsigned int ind_buf_len, void *ind_cb_data )
{
  ...
  if( SNS_SAM_MR_MSG_Q_MAX < sns_q_cnt( &sns_sam_msg_queue ) )
  {
    ...
    /* If the queue is full, remove all Indications from the SAM message queue.
       This is needed in very high concurrency scenarios to reduce SAM load 
       allowing it time to process Requests and Responses */
    sns_sam_mr_remove_all_inds();
#ifdef SNS_QDSP_SIM
    // ASSERT on QDSP SIM to know if samples are being dropped.
    SNS_ASSERT(0);
#endif
  }
  else
...
sns_sam_mr_release_conn( void )
{
  ...
  do
  {
    ...
    if( conn_hndl )
    {
      ...
      {
        uint8_t algoIndex = sns_sam_get_algo_index(conn_hndl->svc_num);
        ...
        err = sns_sam_process_algo_disable_req( clientReqPtr->algoInstId, algoIndex, clientReqId );
        if( err != SNS_SUCCESS )
        {
    ...
    void *service_cookie )
{
  ...
  SNS_SAM_PRINTF3( MEDIUM, "sns_sam_mr_process_req  msg_id %d svc_num %d cxn_hndl %x", msg_id, svc_num, connection_handle );
...
/*!
  ...
  @param[i] send_ind_during_suspend represents if indication should be sent
                           during proc suspend.
...
sns_sam_mr_get_svc_index( uint8_t src_svc_num, uint8_t dst_svc_num )
{
  ...
  // TIME Service can be accessed by any algo - or SAM
  if( SNS_TIME2_SVC_ID_V01 == dst_svc_num )
  {
    rv = SNS_SAM_SVC_ARR_LEN - 1;
  }
  // The REGISTRY, SMGR, SMGR_INTERNAL, PM and MODEM_SCN services 
  // are only accessed from SAM framework, which has a
  ...
  else if( SNS_SAM_MODEM_SCN_SVC_ID_V01 == dst_svc_num )
  {
    rv = 4;
  }
  else
...
sns_sam_mr_send( void *msg_ptr, sns_sam_mr_algo_conn_hndl *algo_conn_hndl )
{
  ...
  // If SAM is trying to send a request on behalf of the framework
  if( NULL == algo_conn_hndl )
  {
...
sns_sam_mr_send_ind(
   ...
   bool send_ind_during_suspend)
{
  UNREFERENCED_PARAMETER(mr_clnt_id);
