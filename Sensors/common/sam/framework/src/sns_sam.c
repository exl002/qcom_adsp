...
*/
/*=======================================================================*/
void sns_sam_add_item_to_buffer(
   uint8_t clientReqId,
   void** batchItemPtr)
{
   ...
   sns_sam_mr_conn_hndl     mrClntId)
{
   ...
   sns_profiling_log_qdss(SNS_SAM_ALGO_REPORT_ENTER, 1, serviceId);
   ...
   if( clientReqPtr->batchInfo != NULL &&
       clientReqPtr->batchInfo->batchPeriod > 0 )
   {
      sns_sam_create_batch_item( clientReqPtr, algoRptPtr, &clientIndMsgPtr, clientReqId );
      sns_sam_add_item_to_buffer( clientReqId, &clientIndMsgPtr );
   }
   else
   {
      err = sns_sam_prep_algo_report_ind(clientReqPtr, &clientIndMsgPtr, algoRptPtr, &msgHdr, mrClntId);
      if( err != SNS_SUCCESS )
      {
   ...
   sns_profiling_log_qdss(SNS_SAM_ALGO_REPORT_EXIT, 1, serviceId);
}
...
/*=========================================================================
  FUNCTION:  sns_sam_get_client_req_report_rate
  =========================================================================*/
/*!
  @brief Fetches the sensor report rate for a client 

  @param[i] clientReqPtr: Pointer to client request
 
  @return   the requested client report rate in Q16 Hz
*/
/*=======================================================================*/
static uint32_t sns_sam_get_client_req_report_rate(const sns_sam_client_req_s *clientReqPtr)
{
   uint32_t client_report_rate = 0;
      ...
      if (clientReqPtr->reportType == SNS_SAM_PERIODIC_REPORT)
      {
         float report_rate = (float)
          (SNS_SAM_USEC_PER_SEC /
           sns_em_convert_localtick_to_usec(clientReqPtr->period));
         client_report_rate = FX_FLTTOFIX_Q16(report_rate);
         SNS_SAM_PRINTF1(MED, "SAM- sns_sam_get_client_req_report_rate "
                                  "client reportType:%d,",
                            clientReqPtr->reportType);
      }
      else
      {
         // sync or async or one shot reporting algo
         if ((clientReqPtr->batchInfo == NULL) ||
             ((clientReqPtr->batchInfo != NULL) &&
             (clientReqPtr->batchInfo->batchPeriod < sns_em_convert_usec_to_localtick(SNS_SAM_SLOW_BATCH_DURATION_US))))
         {
            // client is not batched or defines batching but the period is less than SNS_SAM_SLOW_BATCH_DURATION_US
            ...
            if (clientReqPtr->reportType == SNS_SAM_SYNC_REPORT)
            {
               ...
               {
                  sns_sam_data_req_s * dataReqPtr = sns_sam_data_req_dbase[dataReqId];
                  if (dataReqPtr != NULL)
                  {
                     client_report_rate = FX_CONV_Q16(dataReqPtr->sensorDbase[0].sampleRate,0);
                  }
               }
            }
            else
            {
               ...
               client_report_rate = algoPtr->defSensorReportRate;
            }
         ...
         {
            // client is batched with period requirement in
            // excess of SNS_SAM_SLOW_BATCH_DURATION_US
            sns_sam_algo_s * algoPtr = sns_sam_get_algo_handle(algoInstPtr->serviceId);
            client_report_rate = algoPtr->defSensorReportRate;

            SNS_SAM_PRINTF2(MED, "SAM- sns_sam_get_client_req_report_rate "
                                  "client reportType:%d,"
                                  "client batch period [ticks]:%d",
                            clientReqPtr->reportType,
                            clientReqPtr->batchInfo->batchPeriod);
         }
      }
   }
...
/*=========================================================================
  FUNCTION:  sns_sam_get_max_algo_inst_report_rate
  =========================================================================*/
/*!
  @brief Determines the maximum requested report rate from all the external clients of
         a given algo instance.

  @param[i] algoInstPtr: pointer to algorithm instance

  @return maximum requested rate in Q16 Hz
*/
/*=======================================================================*/
static uint32_t sns_sam_get_max_algo_inst_report_rate(const sns_sam_algo_inst_s *algoInstPtr)
{
   uint8_t i = 0, maxReportClientID = SNS_SAM_INVALID_ID;
   uint32_t maxReportRate = 0, clientReportRate = 0;

   if (algoInstPtr != NULL)
   {
      for (i = 0; i < algoInstPtr->clientReqCount; i++)
      {
         sns_sam_client_req_s * clientReqPtr = sns_sam_client_req_dbase[algoInstPtr->clientReqDbase[i]];

         if ((clientReqPtr != NULL) && (!clientReqPtr->directReportReq))
         {
            // only handle external clients for the algo instance, internal clients that can be identified
            // using the directReportReq flag do not reflect the batching requirements for the  algo chain.
            // For e.g. if GRV is batched this information is present in the client entry for GRV but not in
            // it's dependents Grav & Quat. The reporting requirements for internal client requested algos 
            // like Grav to satisfy the client, GRV, will be captured in the data request requirements 
            // as part of sns_sam_get_max_data_req_report_rate()
            clientReportRate = sns_sam_get_client_req_report_rate(clientReqPtr);
            if (maxReportRate < clientReportRate)
            {
               maxReportRate = clientReportRate;
               maxReportClientID = algoInstPtr->clientReqDbase[i];
            }
            SNS_SAM_PRINTF3(MED, "SAM- sns_sam_get_max_algo_inst_report_rate "
                           "clientReqId:%d, "
                           "clientReportRate[Q16 Hz]:%d, "
                           "maxReportRate[Q16 Hz]:%d",
   ...
   return maxReportRate;
}

/*=========================================================================
  FUNCTION:  sns_sam_get_max_data_req_report_rate
  =========================================================================*/
/*!
  @brief Determines the maximum requested report rate from among all the algo instances
         of a data request

  @param[i] dataReqPtr: pointer to data requesr

  @return report rate in Q16 Hz
*/
/*=======================================================================*/
static uint32_t sns_sam_get_max_data_req_report_rate(const sns_sam_data_req_s *dataReqPtr)
{
   uint8_t i = 0, maxReportAlgoInstID = SNS_SAM_INVALID_ID;
   uint32_t maxReportRate = 0, algoInstReportRate = 0;

   if (dataReqPtr != NULL)
   {
      for (i = 0; i < dataReqPtr->algoInstCount; i++)
      {
         sns_sam_algo_inst_s * algoInstPtr = sns_sam_algo_inst_dbase[dataReqPtr->algoInstDbase[i]];

         if (algoInstPtr != NULL)
         {
            algoInstReportRate = algoInstPtr->sensorReportRate;
            if (maxReportRate < algoInstReportRate)
            {
               maxReportRate = algoInstReportRate;
               maxReportAlgoInstID = dataReqPtr->algoInstDbase[i];
            }
         }
      }
   }
...
/*=========================================================================
  FUNCTION:  sns_sam_update_data_req_report_rate
  =========================================================================*/
/*!
  @brief Updates the report rate of a data request based on report rate requirements
         of algo instances that the data req serves

  @param[i] dataReqId: Index to data request 
 
  @return   None
*/
/*=======================================================================*/
static void sns_sam_update_data_req_report_rate(uint8_t dataReqId)
{
   uint32_t maxReportRate = 0;
   sns_sam_data_req_s *dataReqPtr = NULL;

   if (dataReqId == SNS_SAM_INVALID_ID)
   {
      SNS_SAM_PRINTF0(ERROR, "SAM- sns_sam_update_data_req_report_rate() passed Invalid dataReqId ");
      return;
   }
   ...
   if(dataReqPtr == NULL)
   {
      SNS_SAM_PRINTF0(ERROR, "SAM- sns_sam_update_data_req_report_rate() passed NULL dataReqPtr");
      return;
   }

   maxReportRate = sns_sam_get_max_data_req_report_rate(dataReqPtr);

   if (dataReqPtr->reportRate != maxReportRate)
   {
      SNS_SAM_PRINTF3(MED,"SAM- updating data req Id %d from report rate: %d to report rate: %d",
            dataReqId,
            dataReqPtr->reportRate,
            maxReportRate);

      dataReqPtr->reportRate = maxReportRate;

      SNS_SAM_PRINTF3(MED,"SAM- sns_sam_send_smgr_start_req dataReqId:%d, algoInstId:%d, serviceId:%d ",
            dataReqId,
            dataReqPtr->algoInstDbase[0],
            sns_sam_algo_inst_dbase[dataReqPtr->algoInstDbase[0]]->serviceId);
      sns_sam_send_smgr_start_req(dataReqId,
                                  sns_sam_algo_inst_dbase[dataReqPtr->algoInstDbase[0]]->serviceId);
   }
}
...
/*=========================================================================
  FUNCTION:  sns_sam_update_algo_inst_report_rate
  =========================================================================*/
/*!
  @brief Updates the report rate requirement in a cumulative algo instance,
         based on the maximum required report rate among the external clients
         of the algo instance

  @param[i] algoInstId: Index to algo instance
 
  @return   None
*/
/*=======================================================================*/
static void sns_sam_update_algo_inst_report_rate(uint8_t algoInstId)
{
   sns_sam_algo_inst_s * algoInstPtr = NULL;

   if (algoInstId == SNS_SAM_INVALID_ID)
   {
      SNS_SAM_PRINTF0(ERROR, "SAM- sns_sam_update_algo_inst_report_rate() passed invalid algoInstId");
      return;
   }
   ...
   if (algoInstPtr == NULL)
   {
      SNS_SAM_PRINTF0(ERROR, "SAM- sns_sam_update_algo_inst_report_rate() passed NULL algoInstPtr");
      return;
   }

   if (sns_sam_get_smgr_msg_req_type(algoInstPtr->serviceId) == SNS_SMGR_BUFFERING_REQ_V01)
   {
      uint32_t maxReportRate = algoInstPtr->sensorReportRate;

      SNS_SAM_PRINTF3(MED, "SAM- sns_sam_update_algo_inst_report_rate "
                           "algoInstId:%d, "
                           "serviceId:%d, "
                           "sensorReportRate[Q16 Hz]:%d",
                      algoInstId,
                      algoInstPtr->serviceId,
                      algoInstPtr->sensorReportRate);

      maxReportRate = sns_sam_get_max_algo_inst_report_rate(algoInstPtr);

      if (algoInstPtr->sensorReportRate != maxReportRate)
      {
         uint8_t dataReqId = sns_sam_find_data_req_id(algoInstId,false);
         algoInstPtr->sensorReportRate = maxReportRate;
         sns_sam_update_data_req_report_rate(dataReqId);
      }
   }
}
   ...
   uint8_t algoSvcId)
{
   ...
   if(sns_sam_get_smgr_msg_req_type(algoSvcId) == SNS_SMGR_BUFFERING_REQ_V01)
   {
      ...
      SNS_SAM_PRINTF3(MED, "SAM- Buffering req, algo %d,  %d sensors, rep_rate %d",
   ...
   uint8_t algoInstId)
{
   if( sns_sam_data_req_dbase[dataReqId] != NULL )
   {
      ...
      {
         if (sns_sam_algo_dbase[dataReq->algoInstDbase[0]]->dataSyncRequired)
         {
            dataReq->reportRate = sns_sam_get_max_data_req_report_rate(dataReq);
            dataReqChanged = true;
         }
      }
   ...
   const void *clientReqMsgPtr)
{
   ...
   for (instId = 0; instId < SNS_SAM_MAX_ALGO_INSTS; instId++)
   {
      if (sns_sam_algo_inst_dbase[instId] == NULL)
      {
         ...
         algoInstPtr->sensorReportRate = 0;
   ...
   bool                 directReportReq)
{
   ...
   if (algoInstPtr->clientReqCount > 1)
   {
      // send existing algo instance output to newly added client
     ...
     if (NULL != algoRptPtr)
     {
      ...
      sns_sam_update_algo_inst_report_rate(algoInstId);
   }
   ...
   sns_sam_mr_conn_hndl  mrClntId)
{
   ...
   if( sns_sam_algo_dbase[algoIndex]->dataSyncRequired )
   {
      ...
      if( err == SNS_SUCCESS && dataReq.sensorCount > 0 )
      {
         ...
         // update data request based on client reporting requirement
         if (err == SNS_SUCCESS)
         {
            sns_sam_update_algo_inst_report_rate(algoInstId);
         }
...
#endif
      sns_profiling_log_qdss(SNS_SAM_ALGO_UPDATE_ENTER_1, 1, algoInstPtr->serviceId);
      //execute the algorithm
      sns_sam_algo_dbase[algoIndex]->algoApi.sns_sam_algo_update(
         algoInstPtr->stateData.memPtr,
         algoInstPtr->inputData.memPtr,
         algoInstPtr->outputData.memPtr);
      sns_profiling_log_qdss(SNS_SAM_ALGO_UPDATE_EXIT, 1, algoInstPtr->serviceId);
#ifdef SNS_DSPS_BUILD
   ...
   const void *smgrIndPtr)
{
   ...
   sns_profiling_log_qdss(SNS_SAM_PROCESS_BUFFER_IND_ENTER,1,dataReqId);
   ...
   {
      SNS_SAM_DEBUG1(MEDIUM, DBG_SAM_SMGR_IND_DROPPED, dataReqId);
      sns_profiling_log_qdss(SNS_SAM_PROCESS_BUFFER_IND_EXIT,1,dataReqId);
      return SNS_ERR_FAILED;
   }
   ...
   uint8_t algoSvcId)
{
   ...
   switch (algoSvcId)
   {
      ...
      case SNS_SAM_TILT_DETECTOR_SVC_ID_V01:
         if (sns_sam_sensor_registry_version >= 0x1D)
         {
            retVal = true;
         }
         break;
   ...
   bool wakeClient)
{
   ...
   SNS_SAM_PRINTF3(HIGH,"SAM- Rxd batch request for client id %d, period %d, wakeclient %d",clientReqId, batchPeriod, wakeClient);
   ...
   if (err == SNS_SUCCESS)
   {
      sns_sam_update_algo_inst_report_rate(clientReqPtr->algoInstId);
   }
   ...
   int32_t activeBatchPeriod)
{
   ...
   SNS_SAM_PRINTF2(HIGH,"SAM:Rxd upd batch period request for client id %d, period %d",clientReqId, activeBatchPeriod);
