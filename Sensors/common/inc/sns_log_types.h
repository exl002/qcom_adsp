...
#ifndef LOG_SNS_TILT_DETECTOR_CONFIG_C
  #define LOG_SNS_TILT_DETECTOR_CONFIG_C                     0x1887
#endif
#ifndef LOG_SNS_TILT_DETECTOR_RESULT_C
  #define LOG_SNS_TILT_DETECTOR_RESULT_C                     0x1888
#endif
#ifndef LOG_SNS_TILT_DETECTOR_REPORT_C
  #define LOG_SNS_TILT_DETECTOR_REPORT_C                     0x1889
#endif
...
typedef PACK(struct) {
   ...
   int32_t      sensor_report_rate; /**< Requested sensor report rate in Hz, Q16 */
} sns_log_qmd_config_s;
...
#define SNS_LOG_QMAG_CAL_CONFIG_VERSION 255
typedef PACK(struct) {
   log_hdr_type log_type;              /**< Type, as defined in log_codes.h */
   uint8_t      version;               /**< Version of the log packet       */
   uint8_t      timestamp_type;        /**< type of timestamp               */
...
#define SNS_LOG_QMAG_CAL_RESULT_VERSION 255
typedef PACK(struct) {
   log_hdr_type log_type;         /**< Type, as defined in log_codes.h */
   uint8_t      version;          /**< Version of the log packet       */
   uint8_t      timestamp_type;        /**< type of timestamp               */
...
#define SNS_LOG_QMAG_CAL_REPORT_VERSION 255
typedef PACK(struct) {
   log_hdr_type log_type;              /**< Type, as defined in log_codes.h */
   uint8_t      version;               /**< version of the log packet */
   uint8_t      timestamp_type;        /**< type of timestamp               */
...
#define SNS_LOG_CMC_RESULT_VERSION 2
typedef PACK(struct) {
  log_hdr_type log_type;
  uint8_t      version;                      /* version of the log packet */
  uint8_t      timestamp_type;               /* type of timestamp          */
  ...
  uint8_t      input_type;
  int32_t      accel[3];
  uint8_t      qmd_motion_state;
  uint8_t      update_report_motion_state;   /* The state for which to apply the update_report_type */
  uint8_t      update_report_type;           /* whether to enable/disable entry/exit reporting of the specified state*/
  uint8_t      algo_inst_id;                 /* algorithm instance id   */
} sns_log_cmc_result_s;

#define SNS_LOG_CMC_RESULT2_VERSION 2
typedef PACK(struct) {
   log_hdr_type log_type;
   uint8_t      version;                      /* version of the log packet */
   uint8_t      timestamp_type;               /* type of timestamp          */
   uint64_t     timestamp;                    /* timestamp in SSC ticks*/
   float        motion_feature[34];
   float        unfilt_gmm_prob[5];
   float        unfilt_lr_prob_stat_dr[2];
   float        unfilt_lr_prob_walk_bike[2];
   float        unfilt_prob[5];
   float        filt_prob[5];
   uint8_t      hyst_stable_state;
   uint8_t      hyst_transition_state;
   uint8_t      hyst_is_transition;
   uint16_t     hyst_transition_count;
   uint8_t      event_report_types[6];
   uint8_t      motion_event[6];
   uint32_t     ms_timestamp[6];
   float        ms_probability[6];
   uint8_t      op_motion_event[6];
   uint32_t     op_ms_timestamp[6];
   float        op_ms_probability[6];
   uint8_t      algo_inst_id;                 /* algorithm instance id   */
} sns_log_cmc_result2_s;

#define SNS_LOG_CMC_REPORT_VERSION 2
typedef PACK(struct) {
  log_hdr_type log_type;
  uint8_t      version;                         /* version of the log packet */
  uint8_t      timestamp_type;                  /* type of timestamp          */
...
typedef struct {
  ...
  float        ms_bike_speedbound;        /**< speed bound for bike state                     */
  uint8_t      algo_inst_id;              /**< Algorithm instance id                          */
} sns_log_distance_bound_config_s;

#define SNS_LOG_DISTANCE_BOUND_RESULT_VERSION 2
typedef PACK(struct) {
  log_hdr_type log_type;                    /**< Type, as defined in log_codes.h                */
  uint8_t      version;                     /**< Version of the log packet                      */
  uint8_t      timestamp_type;              /**< Type of timestamp                              */
  ...
  uint8_t      input_cmc_motion_event;      /**< Motion Event input from CMC                    */
  uint8_t      algo_state;                  /**< 0: inactive 1: active                          */
  uint8_t      prev_cmc_motion_state;       /**< MS used at last distance bound computation     */
  ...
  uint8_t      is_simultaneous_state;       /**< true: CMC last reported simultaneous states; false: CMC did not last report simultaneous states  */
  uint8_t      prev_simultaneous_cmc_state; /**< Simultaneous state reported by CMC along with previous_cmc_motion_state */
...
#define SNS_LOG_SMD_CONFIG_VERSION 1
typedef PACK(struct) {
  log_hdr_type log_type;
  uint8_t     version;                /* Version                                        */
  uint8_t     timestamp_type;         /* type of timestamp                              */
  ...
  int32_t     accel_window_time;      /* accel window time, sec                         */
  int32_t     detect_threshold;		    /* accel detect threshold, m/s/s, Q16             */
  int32_t     self_transition_prob_nm;/* Self transition probability of no motion, Q16  */
  uint8_t     variable_latency_flag;  /* Flag to indicate variable latency, 0 or 1      */                      
  uint8_t     max_latency;            /* Maximum latency if fixed latency is used, sec  */ 
  uint8_t     algo_inst_id;           /* algo instance id                               */
} sns_log_smd_config_s; 

#define SNS_LOG_SMD_RESULT_VERSION 1
typedef PACK(struct) {
  log_hdr_type log_type;
  uint8_t   version;                    /* Version                                */
  uint8_t   timestamp_type;             /* type of timestamp                      */
  ...
  int32_t   eigenvalues[3];	            /* eigenvalues, Q16                       */
  int32_t   xcor;							          /* auto correlation, Q16                  */
  int32_t   spread_mag;					        /* spread magnitude, Q16                  */
  int32_t   rm;							            /* RM Ratio feature, Q16                  */
  int32_t   sa;							            /* SA feature (Std dev of norm), Q16      */
  ...
  int8_t    motion_state;					      /* trasient motion state, 0, 1 or 2       */
  int8_t    final_motion_state;					// final motion state, reported to client */
  uint8_t   algo_inst_id;               /* algo instance id                       */
} sns_log_smd_result_s; 

#define SNS_LOG_SMD_REPORT_VERSION 1
typedef PACK(struct) {
  log_hdr_type log_type;
  uint8_t      version;                   /* Version                                  */
  uint8_t      timestamp_type;            /* type of timestamp                        */
  ...
  uint8_t      report_type;               /* report type                              */
  uint8_t      client_id;                 /* client id                                */
  int8_t       motion_state;				      /* motion state                             */
  uint8_t      motion_state_probability;	/* probability of the motion state (0-100)  */
  uint8_t      algo_inst_id;              /* algo instance id                         */
}sns_log_smd_report_s;
...
#define SNS_LOG_TILT_DETECTOR_CONFIG_VERSION 1
typedef PACK(struct) {
  log_hdr_type log_type;                  /**< Type, as defined in log_codes.h                   */
  uint8_t      version;                   /**< Version of the log packet                         */
  uint8_t      timestamp_type;            /**< Type of timestamp                                 */
  uint64_t     timestamp;                 /**< Time in SSC ticks when algorithm is configured    */
  float        sample_rate;               /**< Rate at which the sensors are sampled, Hz         */
  float        angle_threshold;           /**< radians             */
  float        init_accel_window_time;    /**< time window to average accel for the initial gravity vector, sec */
  float        accel_window_time;         /**< Time window to average accel for the current gravity vector, sec */
  uint8_t      algo_inst_id;              /**< Algorithm instance id                             */
} sns_log_tilt_detector_config_s;

#define SNS_LOG_TILT_DETECTOR_RESULT_VERSION 1
typedef PACK(struct) {
  log_hdr_type log_type;                 /**< Type, as defined in log_codes.h      */
  uint8_t      version;                  /**< Version of the log packet            */
  uint8_t      timestamp_type;           /**< type of timestamp                    */
  uint64_t     timestamp;                /**< timestamp of Tilt result             */
  int32_t      accel[3];                 /**< accel input, m/s/s Q16               */
  uint64_t     accel_timestamp;          /**< time stamp in SSC ticks when the accel input was generated */
  float        dprod_threshold;          /**< dot product threshold                */
  float        reference_gravity[3];     /**< reference gravity vector, m/s/s      */
  uint8_t      algo_inst_id;             /**< algorithm instance id                */
} sns_log_tilt_detector_result_s;

#define SNS_LOG_TILT_DETECTOR_REPORT_VERSION 1
typedef PACK(struct) {
  log_hdr_type log_type;                 /**< Type, as defined in log_codes.h      */
  uint8_t      version;                  /**< Version of the log packet            */
  uint8_t      timestamp_type;           /**< type of timestamp                    */
  ...
  uint8_t      report_type;              /**< report type                          */
  uint8_t      client_id;                /**< client id                            */
  uint32_t     event_ts;                 /**< reported event timestamp, SSC ticks  */
  float        new_reference_gravity[3]; /**< New reference gravity vector, m/s/s  */
  uint8_t      algo_inst_id;             /**< algorithm instance id                */
}sns_log_tilt_detector_report_s;
