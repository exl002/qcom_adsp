...
/*==============================================================================
  ...
  $Header: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/ddf/src/8974/sns_ddf_signal.c#3 $
  $DateTime: 2014/08/01 17:40:51 $

  when       who  what, where, why
  ---------- ---  -----------------------------------------------------------
  2104-07-30 sd   Fixed GPIO config for interrupt
...
sns_ddf_status_e sns_ddf_signal_deregister (uint32_t gpio_num)
{
    ...
    sns_ddf_sig_tb.sig_info[sig_idx].gpio_pin_cfg = DAL_GPIO_CFG(gpio_num, 0, DAL_GPIO_INPUT, DAL_GPIO_NO_PULL, DAL_GPIO_2MA);
    DalTlmm_SetInactiveConfig(sns_ddf_sig_tb.tlmm_handle, gpio_num, sns_ddf_sig_tb.sig_info[sig_idx].gpio_pin_cfg);
    status = DalTlmm_ConfigGpio(sns_ddf_sig_tb.tlmm_handle, sns_ddf_sig_tb.sig_info[sig_idx].gpio_pin_cfg, DAL_TLMM_GPIO_DISABLE);
    if ( DAL_ERROR == status )
    {
        DDF_MSG_1(FATAL, "inactive_config - GPIO Num %d failed", gpio_num);
    }
#endif
