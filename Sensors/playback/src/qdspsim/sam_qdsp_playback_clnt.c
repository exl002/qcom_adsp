...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2014-05-31  tc   Added Tilt Detector algorithm
...
/*=========================================================================
  FUNCTION: sns_plbk_sam_tilt_detector_enable
=========================================================================*/
/*!
    @brief
    Enable's a SAM Tilt Detector Algorithm by sending enable message.
*/
/*=======================================================================*/
int sns_plbk_sam_tilt_detector_enable(int *algo_config,
                            qmi_client_type *clnt)
{
    qmi_txn_handle txn;
    qmi_client_type notifier;
    ...
    qmi_service_info service_info;
    qmi_idl_service_object_type service_object =
                                      SNS_SAM_TILT_DETECTOR_SVC_get_service_object_v01();
    sns_sam_tilt_detector_enable_req_msg_v01 *data_req;
    sns_sam_tilt_detector_enable_resp_msg_v01 *data_resp;
    int rc;

    if (!service_object) {
        printf("TILT CLNT: get_service_object failed\n");
    }
    ...
    printf("TILT CLNT: qmi_client_get_service_list() returned %d num_entries=%d"
           " num_services = %d\n", rc, num_entries, num_services);
    if (rc != QMI_NO_ERR) {
        return -1;
    }
    ...
    rc = qmi_client_init(&service_info, service_object, sam_ind_cb, NULL,
                         &os_params, clnt);
    printf("TILT CLNT: qmi_client_init returned %d\n", rc);
    if (rc != QMI_NO_ERR) {
        return -1;
    }

    /* Prepare and send the request */
    data_req = (sns_sam_tilt_detector_enable_req_msg_v01*)
                                MALLOC(sizeof(sns_sam_tilt_detector_enable_req_msg_v01));
    data_resp = (sns_sam_tilt_detector_enable_resp_msg_v01*)
                               MALLOC(sizeof(sns_sam_tilt_detector_enable_resp_msg_v01));

    if (!data_req || !data_resp) {
        return -1;
    }

    memset( data_req, 0, sizeof(sns_sam_tilt_detector_enable_req_msg_v01) );
    memset( data_resp, 0, sizeof(sns_sam_tilt_detector_enable_resp_msg_v01) );

    /* sam Tilt Detector config */
    data_req->angle_thresh = algo_config[0];

    rc = qmi_client_send_msg_async(*clnt, SNS_SAM_TILT_DETECTOR_ENABLE_REQ_V01, data_req,
              sizeof(sns_sam_tilt_detector_enable_req_msg_v01), data_resp,
              sizeof(sns_sam_tilt_detector_enable_resp_msg_v01), sam_rx_cb, NULL, &txn);
    printf("TILT CLNT: qmi_client_send_msg_async returned %d\n", rc);
    if (rc != QMI_NO_ERR) {
        return -1;
    }
    return 0;
...
/*=========================================================================
  FUNCTION: sns_plbk_sam_tilt_detector_disable
=========================================================================*/
/*!
    @brief
    Disabble's a SAM Tilt Detector Algorithm by sending disable message.
*/
/*=======================================================================*/
int sns_plbk_sam_tilt_detector_disable(qmi_client_type *clnt)
{
    sns_sam_tilt_detector_disable_req_msg_v01 *data_req;
    sns_sam_tilt_detector_disable_resp_msg_v01 *data_resp;
    qmi_txn_handle txn;
    int rc;

    /* Prepare and send the request */
    data_req = (sns_sam_tilt_detector_disable_req_msg_v01*)
                               MALLOC(sizeof(sns_sam_tilt_detector_disable_req_msg_v01));
    data_resp = (sns_sam_tilt_detector_disable_resp_msg_v01*)
                              MALLOC(sizeof(sns_sam_tilt_detector_disable_resp_msg_v01));

    if (!data_req || !data_resp) {
        return -1;
    }
    memset( data_req, 0, sizeof(sns_sam_tilt_detector_disable_req_msg_v01) );
    memset( data_resp, 0, sizeof(sns_sam_tilt_detector_disable_resp_msg_v01) );

    data_req->instance_id = algo_instance_id;

    rc = qmi_client_send_msg_async(*clnt, SNS_SAM_TILT_DETECTOR_DISABLE_REQ_V01, data_req,
               sizeof(sns_sam_tilt_detector_disable_req_msg_v01), data_resp,
               sizeof(sns_sam_tilt_detector_disable_resp_msg_v01), sam_rx_cb, NULL, &txn);
    if (rc != QMI_NO_ERR) {
        return -1;
    }
    return 0;
