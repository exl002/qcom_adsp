...
/*==============================================================================
  ...
  $Header: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/dd/qcom/src/sns_dd_inertial_test.c#1 $
...
static void sns_dd_inertial_test_get_data(sns_dd_inertial_test_s* state)
{
    ...
    sns_ddf_memhandler_free(&state->memhandler);
}
...
static void sns_dd_inertial_test_handle_timer(sns_ddf_handle_t handle, void* arg)
{
    ...
    gyro_cal_scm_update(state->algo_state, &state->algo_input, &state->algo_output);
    ...
    sns_dd_inertial_test_config_s* config)
{
    ...
    algo_size = gyro_cal_scm_mem_req(&algo_config);
    if(algo_size == 0) {
        state->status = SNS_DDF_EINVALID_PARAM;
        return;
    }
    ...
    gyro_cal_scm_state_reset(&algo_config, state->algo_state);
}
