...
/*==============================================================================
  ...
  ---------- ---  -----------------------------------------------------------
  2014-08-13  sd  do not reset odr counter when setting sensor odr to 0
  2014-07-29  sd  mark beggining data invalid for pressure sensor when odr changes
  2014-07-24  sd  Recover from bad timestamp
  2014-07-11  sd  enabled SC/SD/SMD
  2014-06-24  sd  updated set watermark function
  2014-05-30  sd  Fixed accel/gyro stops indication when pressure sensor is deleted
  2014-05-19 jms  Fixed timestamp issues. Flushing all samples from fifo. TS validation.
  2014-05-15  sd  Make DMP FIFO sensors ( except mag/pressure) have same ODR
  2014-04-25  MW  Eliminate compiler warning
  2014-04-15 lka  Fixed ODR issue when disabling MD interrupt.
  2014-04-15 lka  Mapped output of Game Rotation Vector to proper axes.
                  Removed compiler warnings.
  2014-04-04 lka  Fixed register configuration when disabling self-test.
...
/* is it Sensor sharing FIFO except MAG and Pressure*/
#define DMP_IS_ORIGIN_FIFO_SENSOR(s)  (( s == SNS_DDF_SENSOR_GAME_RV) || ( s == SNS_DDF_SENSOR_ACCEL ) || ( s == SNS_DDF_SENSOR_GYRO ) || \
                                       ( s == SNS_DDF_SENSOR_STEP_EVENT ) || ( s == SNS_DDF_SENSOR_STEP_COUNT ))
/*
...
/**
 * @brief Searches through all DMP FIFO sensors(except mag/pres)
 *        within the device instance to find the highest ODR.
 *
 * @param[in]   state     Pointer to the device-instance.
 *
 * @return      The maximum  ODR.
 */
uint32_t mpu6515_find_highest_odr(mpu6515_s* state)
{
  uint8_t i;
  uint32_t j;
  uint32_t max_odr = 0;
  ...
  for (i = 0; i < state->sub_device_cnt; i++)
  {
    for (j = 0; j < state->sub_devices[i].num_sensors; j++)
    {
      if( DMP_IS_ORIGIN_FIFO_SENSOR(state->sub_devices[i].sensors[j].sensor_type)
          && (max_odr < state->sub_devices[i].sensors[j].odr) )
      {
        max_odr = state->sub_devices[i].sensors[j].odr;
      }
    }
...
/**
 * @brief Searches through all DMP FIFO sensors (except 
 *        mag/pres) within the device instance to find the
 *        highest ODR.
 *
 * @param[in]   state     Pointer to the device-instance.
 *
 * @return      The maximum desired ODR.
 */
uint32_t mpu6515_find_highest_desired_odr(mpu6515_s* state)
{
  uint8_t i;
  uint32_t j;
  uint32_t max_odr = 0;
    ...
    mpu6515_sensor_s* sensor, uint32_t odr)
{
  ...
  uint8_t dmp_sensor, k;
  uint32_t target_odr;
  bool reset_odr_cnt = true;

  dmp_sensor = sns_dd_dmp_get_sensor(sensor->sensor_type);
  sensor->init_settle_timestamp = 0;
  
  MPU6050_MSG_3(MED, "set_dmp_odr: ddf_sensor=%u dmp_sensor=%u odr=%u", sensor->sensor_type, dmp_sensor, odr);
    ...
    //if(sensor->odr != target_odr)
    {
      ...
      switch(dmp_sensor)
      {
        ...
        case DMP_SENSOR_STEPCNT:
          ...
          break;
        case DMP_SENSOR_PRESSURE:
          sensor->init_settle_timestamp = sns_ddf_get_timestamp() + sns_ddf_convert_usec_to_tick(MPU6XXX_PRESSURE_READY_TIME_AFTER_ODR_SET_US);
        case DMP_SENSOR_RAW_ACCEL:
        case DMP_SENSOR_RAW_GYRO:
        case DMP_SENSOR_6QUAT:
        case DMP_SENSOR_COMPASS:
      ...
      // Resetting ODR counters for sample alignment within DMP
      // and reduction of FIFO interrupts
      if (reset_odr_cnt)
      {
         for(k = 1; k < DMP_NUM_SENSOR; k++)
         {
            if(dmp_sensor_map[k].hdr != DMP_HDR_INVALID &&
                    dmp_sensor_map[k].ddf_sensor != SNS_DDF_SENSOR__NONE &&
                     dmp_sensor_map[k].odr_cntr != 0)
            {
              status = mpu6515_write_dmp_reg(state, DMP_GET_ODR_CNTR_ADDR(k), bytes_clr_odr_cntr, sizeof(bytes_clr_odr_cntr));
              if(status != SNS_DDF_SUCCESS)
                  return status;
            }
         }
...
{
  sns_ddf_status_e status = SNS_DDF_SUCCESS;
  uint32_t  odr_to_configure = 0, iter = 0 , current_iter = 0;
  bool     odr_update = false;

  MPU6050_MSG_3(MED, "set_odr desired_odr=%u sensor=%u dmp_sensor_bitmap=0x%x", odr, 
                                   sensor->sensor_type, dmp_sensor_bitmap);

  // Max ODR, as physically limited by the device, is 200 Hz
  sensor->desired_odr = MIN(odr,MPU6500_DOUBLETAP_MAX_ODR);
  odr_to_configure =sensor->desired_odr;

  if(sensor->desired_odr )
  {
    /* MAG/pressure doesn't have same ODRs as others, SMD doesn't use FIFO */
    if( (sensor->sensor_type !=SNS_DDF_SENSOR_MAG) && 
        (sensor->sensor_type !=SNS_DDF_SENSOR_PRESSURE) &&
        (sensor->sensor_type !=SNS_DDF_SENSOR_SMD ) &&
         !DMP_IS_SENSOR_ENABLED(DMP_SENSOR_COMPASS) &&
         !DMP_IS_SENSOR_ENABLED(DMP_SENSOR_PRESSURE ))
    {
       /* odrs used in FIFO */
       odr_to_configure = MAX(odr, mpu6515_find_highest_odr(state));
       odr_update = true;
    }
  }

  // Set the ODR of DMP sensor
  status = mpu6515_set_dmp_odr(state, sensor, odr_to_configure);
  ...
  if( odr_to_configure==0 ) 
  {
    if(!DMP_IS_SENSOR_ENABLED(DMP_SENSOR_COMPASS) &&
         !DMP_IS_SENSOR_ENABLED(DMP_SENSOR_PRESSURE ))
    {
      odr_to_configure = mpu6515_find_highest_desired_odr(state);
      odr_update = true;
   }
  }
  ...
  if( odr_update )
  {
    ...
    {  
      ...
      if(dmp_sensor_bitmap & current_iter)
      {
        ...
        if((sensor_ptr->odr !=0) && (sensor_ptr->odr !=odr_to_configure) )
        {
          status = mpu6515_set_dmp_odr(state, sensor_ptr, odr_to_configure);
          if (status != SNS_DDF_SUCCESS) 
          {
            MPU6050_MSG_2(ERROR, "set_dmp_odr: sensor_type=%d,error=%u",
                                           sensor_ptr->sensor_type, status);
          }
          else
          {
            mpu6515_sub_dev_s * sub_device;
            mpu6515_search_for_sensor(state, sensor_ptr->sensor_type, &sub_device);
            sns_ddf_smgr_notify_event(
                    sub_device->smgr_handle,
                    sensor_ptr->sensor_type,
...
sns_ddf_status_e sns_dd_mpu6515_config_standby(mpu6515_s* state)
{
  ...
  mpu6515_sensor_s *sensor_sc_ptr, *sensor_sd_ptr, *sensor_smd_ptr;
  sns_ddf_status_e status = SNS_DDF_SUCCESS;
  uint8_t bytes_written;
  ...
  if(!state->md_enabled)
  {
    ...
    sensor_sd_ptr     = mpu6515_search_for_sensor(state, SNS_DDF_SENSOR_STEP_EVENT, NULL);
    sensor_sc_ptr     = mpu6515_search_for_sensor(state, SNS_DDF_SENSOR_STEP_COUNT, NULL);
    sensor_smd_ptr    = mpu6515_search_for_sensor(state, SNS_DDF_SENSOR_SMD, NULL);
    ...
    else if( (sensor_sd_ptr != NULL) && (sensor_sd_ptr->odr > 0) ||
             (sensor_sc_ptr != NULL) && (sensor_sc_ptr->odr > 0) ||
             (sensor_smd_ptr != NULL) && (sensor_smd_ptr->odr > 0) )
    {
        state->mpu_reg[MPU_REG_PWR_MGMT_2] &= ~(STBY_XA_ENABLE | STBY_YA_ENABLE | STBY_ZA_ENABLE);
        // assuming [0] is accel... workaround
        state->sub_devices[0].powerstate = SNS_DDF_POWERSTATE_ACTIVE;
   ...
   void*               value)
{
  ...
  switch(attrib)
  {
    ...
    case SNS_DDF_ATTRIB_ODR:
    {
      ...
      status = SNS_DDF_SUCCESS;
      mpu6515_sensor_s *sensor_accel = mpu6515_get_sensor(dd_handle, SNS_DDF_SENSOR_ACCEL);
      ...
      if(*odr > 0 && state->md_enabled)
      {
        ...
        if(status == SNS_DDF_SUCCESS)
        {
          ...
          {
            status = mpu6515_set_odr(state, sensor_accel, state->md_accel_odr);
            if(status != SNS_DDF_SUCCESS)
              return status;
          }

          /* sets ODR of original sensor */
 ...
 */
uint16_t sns_dd_dmp_read_fifo(
    mpu6515_s* state,
    uint32_t max_bytes2read,
    uint32_t min_bytes2read,
    uint8_t* buf
)
{
  uint8_t buffer[2] = { 0 };
  ...
  byte_cnt = MIN(fifo_cnt, max_bytes2read);
  if ( byte_cnt < min_bytes2read )
  {
    //MPU6050_MSG_2( MED, "read_fifo  byte_cnt=%u < minB=%u", byte_cnt, min_bytes2read);
    return 0;
  }
...
/**
 * @brief Calculate number of invalid samples
 *
 * @param[i] first_ts: First sample timestamp
 * @param[i] odr_settle_ts: The timestamp when sample becomes 
 *       valid after ODR change
 * @param[i] end_ts: last sample timestamp
 * @param[i] num_samples: number of samples
 *
 * @return  Number of invalid samples
 */
static uint16_t mpu6515_num_invalid_samples(sns_ddf_time_t first_ts, 
                                            sns_ddf_time_t odr_settle_ts, 
                                            sns_ddf_time_t end_ts, 
                                            uint8_t num_samples)
{
  uint16_t num_invalid_samples = 0;
  
  if(( int32_t)(end_ts - first_ts) < 0)
  {
    MPU6050_MSG_2(ERROR, "invalid timestamp first_ts=%d, end_ts=%d", 
                     first_ts, end_ts);
  }
  if ( (int32_t)(odr_settle_ts - first_ts) <= 0 )
  {
    /* all samples in FIFO produced after ODR change settled */
    num_invalid_samples = 0;
  }
  else if ( (int32_t)(odr_settle_ts - end_ts) > 0 )
  {
  ...
  else /* some samples in FIFO produced before ODR change settled */
  {
    if( num_samples>1 ) 
    {
      uint32_t  time_interval = (end_ts - first_ts)/(num_samples-1); 
      num_invalid_samples = 
        ((odr_settle_ts - first_ts) / time_interval) + 1;
    }
    else
    {
      MPU6050_MSG_3(HIGH, "invalid timestamp for %d sample %d first_ts=%d, end_ts=%d", 
                    num_samples, first_ts, end_ts);
    }
  }
...
/**
 * @brief send fifo data to sensor manager
 *
 * @param[i] state: sensor state
 * @param[i] dmp_sensor: DMP sensor
 * @param[i] sensor_ptr: sensor pointer 
 * @param[i] start_pkt: start packet number for this sensor in 
 *       parsed FIFO
 * @param[i] num_samples: number of samples
 *
 * @return  None
 */
static void mpu6515_send_fifo_data(mpu6515_s* state, uint8_t dmp_sensor,mpu6515_sensor_s* sensor_ptr,
                                    uint8_t start_pkt, uint8_t num_samples )
{
  sns_ddf_handle_t smgr_handle = NULL;
  parsed_fifo_info_s * fifo_info_ptr = &parsed_fifo_info[dmp_sensor];
  uint8_t dimension = dmp_sensor_map[dmp_sensor].dim, pkt=0, dim=0;
  for (pkt=0; pkt<num_samples; pkt++)
    {
      uint8_t* dev_fifo_ptr = &dev_fifo[fifo_info_ptr->pkt_start_idx[start_pkt+pkt]];
      for (dim=0; dim<dimension; dim++)
      {
        ...
        case DMP_SENSOR_STEPDET:
          {
            state->data_cache_time = state->sensor_data.end_timestamp;
            if(DMP_IS_SENSOR_ENABLED(DMP_SENSOR_STEPDET))
            {
...
static void mpu6515_consume_fifo_data(
    mpu6515_s*     state,
    sns_ddf_time_t irq_timestamp,
    sns_ddf_time_t fifo_read_timestamp,
    uint8_t        dmp_sensor_trig_intr
)
{
  uint8_t dmp_sensor;
  uint16_t num_invalid_samples = 0;
  static uint32_t  num_bad_gap = 0;
  ...
  for (dmp_sensor=0; dmp_sensor<ARR_SIZE(parsed_fifo_info); dmp_sensor++)
  {
    ...
    sns_ddf_time_t timestamp         = irq_timestamp;
    sns_ddf_time_t delta_ts          = 0; //delta time ticks from last TS to current TS
    sns_ddf_time_t approx_delta_ts   = 0; //delta time ticks approximated from last_ts and odr
    sns_ddf_time_t lwr_appx_delta_ts = 0; //lower approximation from last_ts and odr
    sns_ddf_time_t approx_ts         = 0;
    sns_ddf_time_t read_delta_ts     = 0;
    sns_ddf_time_t interval_ts        =0;
    ...
    if(dmp_sensor == DMP_SENSOR_STEPDET ) 
    {
      MPU6050_MSG_1(HIGH, "parsed: step detected, sensor=%u", dmp_sensor);
    }
    else if ( sensor_ptr->odr == 0 )
    {
      MPU6050_MSG_4(MED, "ODR=0 ts=%u thres=%u sens=%d #pkts=%u", timestamp,
              state->current_thres, sensor_ptr->sensor_type, fifo_info_ptr->num_pkts);
      continue;
    }

    //--------------------- Timestamp calc ---------------------------

    //Base on ODR and num_pkts approximating delta timestamp for all the samples in fifo
    if ( fifo_info_ptr->num_pkts > 1 )
    {
      approx_delta_ts   =  //approx delta_ts for pkts read from fifo
          TICKS_PER_SEC*((sns_ddf_time_t)fifo_info_ptr->num_pkts)   / sensor_ptr->odr;
      lwr_appx_delta_ts =
          TICKS_PER_SEC*((sns_ddf_time_t)fifo_info_ptr->num_pkts-1) / sensor_ptr->odr;
    }
    else
    {
      approx_delta_ts   = ODR_INTERVAL_TICKS(sensor_ptr->odr);
      lwr_appx_delta_ts = ODR_INTERVAL_TICKS(sensor_ptr->odr)>>2;
    }

    if ( sensor_ptr->last_sample_timestamp == 0 ) // first time of last_ts
    {
      sensor_ptr->last_sample_timestamp = timestamp -
          TICKS_PER_SEC*((sns_ddf_time_t)fifo_info_ptr->num_pkts+1) / sensor_ptr->odr;
    }

    //fifo delta timestamp  based on last_ts and (irq_ts or trigger_fifo_ts)
    delta_ts      = timestamp - sensor_ptr->last_sample_timestamp;
    approx_ts     = sensor_ptr->last_sample_timestamp + approx_delta_ts;
    read_delta_ts = fifo_read_timestamp - sensor_ptr->last_sample_timestamp;

     /*MPU6050_MSG_4(MED, "## dts=%u  apx_dts=%u read_dts=%u sensor=%d", delta_ts,
                   approx_delta_ts, read_delta_ts, sensor_ptr->sensor_type );
     MPU6050_MSG_4(MED, "## #pkts=%u (Expect#pkts intr=%u/10 read=%u/10) thres=%u",
      fifo_info_ptr->num_pkts, 10*delta_ts * sensor_ptr->odr / TICKS_PER_SEC,
      10*read_delta_ts * sensor_ptr->odr / TICKS_PER_SEC, state->current_thres );
      */
    //--------------------- Timestamp validator ------------------------
    //- Accepts ts roll-overs. At ts roll-over delta_ts are correct.
    //- If the timestamp from read is incorrect use ts estimated from ODR.
    //- If the timestamp from irq is incorrect from read.

    if ( timestamp == fifo_read_timestamp ) //timestamp from triger_fifo not irq
    {
      timestamp = approx_ts; //use only estimated ts
    }
    else //called from irq sig dispatch (timestamp from irq)
    {
      sns_ddf_time_t max_delta_ts = TICKS_PER_SEC*MPU6515_MAX_PKTS_IN_FIFO/sensor_ptr->odr;
      //validate read_dts
      if ( read_delta_ts < lwr_appx_delta_ts  )
      {
        MPU6050_MSG_3( MED, "TSV1 use apx_ts, sens=%d  r_dts=%u < (apx_ts-ODR/2)=%u",
               sensor_ptr->sensor_type,  read_delta_ts, lwr_appx_delta_ts );

        timestamp = approx_ts;  //read_ts incorrect ues approximated ts
      }
      else if( read_delta_ts > max_delta_ts ) 
      { 
        // too much gap, need to recover
        num_bad_gap++;
        MPU6050_MSG_4( MED, "TSV1 use apx_ts, sens=%d max_dts=%u < r_dts=%u num_bad_gap=%u",
               sensor_ptr->sensor_type, max_delta_ts, read_delta_ts, num_bad_gap );
        timestamp = fifo_read_timestamp;
      }
      else  //read_ts OK, validate irq_ts
      {
        sns_ddf_time_t diff_ts = fifo_read_timestamp - timestamp;
        if ( diff_ts > UPPER_ODR_INTERVAL_TICKS(sensor_ptr->odr) )
        {
          //irq was processed later than ODR interval from intr ts
          //- Read fifo (read_ts) should be executed no later than one ODR interval from INTR
          //  or new samples may get into fifo making irq_ts inaccurate.
          //- irq_ts has to be older then read_ts
          MPU6050_MSG_4(MED, "TSV2 use read_ts, sens=%d r_dts=%u (r_ts-irq_ts)=%u > ODRtks=%u",
                        sensor_ptr->sensor_type,read_delta_ts,
                        diff_ts, UPPER_ODR_INTERVAL_TICKS(sensor_ptr->odr) );
          //irq_ts incorrect use read_ts-1/2 odr
          timestamp = fifo_read_timestamp- UPPER_ODR_INTERVAL_TICKS(MAX(sensor_ptr->odr, 10))/2; 
        }
        else
        if ( delta_ts < lwr_appx_delta_ts || delta_ts > max_delta_ts )
        {
          //- Interval last_ts to irq_ts is too short or longer than predicted from
          //  ODR_interval*pkts
          //    - loosing too many samples
          //    - interval could not be calculated <=> (last_ts > irq_ts)
          MPU6050_MSG_4(MED,"TSV3 use read_ts, sens=%d lwr_apx_dts=%u > dts=%u > max_dts=%u",
                        sensor_ptr->sensor_type, lwr_appx_delta_ts, delta_ts, max_delta_ts);

          timestamp = fifo_read_timestamp; //irq_ts incorrect use read_ts
        }
        else //irq_ts OK use it!
        {
          //MPU6050_MSG_0(MED, "### use IRQ_TS ###");
          timestamp = irq_timestamp;
          /*if ( dmp_sensor == dmp_sensor_trig_intr ) {
            MPU6050_MSG_3(MED, "INTR SRC sensor=%d  ODR_intvl_mpu=%u  ODR_intvl=%u",
                          sensor_ptr->sensor_type, delta_ts / fifo_info_ptr->num_pkts,
                          ODR_INTERVAL_TICKS(sensor_ptr->odr) );
          }*/
        }
      }
    }
    ...
    // num_pkts was checked nun zero earlier in this function
    interval_ts = (timestamp - sensor_ptr->last_sample_timestamp)/ fifo_info_ptr->num_pkts;
    
    if (fifo_info_ptr->num_pkts > 1)
    {
       state->sensor_data.timestamp = sensor_ptr->last_sample_timestamp +interval_ts;
    }
    else
    {
    ...
    /* sensor specific */
    if( dmp_sensor == DMP_SENSOR_PRESSURE )
    {
      num_invalid_samples = mpu6515_num_invalid_samples(state->sensor_data.timestamp, 
                                     sensor_ptr->init_settle_timestamp, timestamp,
                                                      fifo_info_ptr->num_pkts );
    }
    state->sensor_data.sensor = DMP_GET_DDF_SENSOR(dmp_sensor);
    if( num_invalid_samples )
    {
      state->sensor_data.status = SNS_DDF_EINVALID_DATA;
      state->sensor_data.num_samples = num_invalid_samples * dmp_sensor_map[dmp_sensor].dim;
      if( num_invalid_samples <fifo_info_ptr->num_pkts ) 
      {
        state->sensor_data.end_timestamp = sensor_ptr->last_sample_timestamp +
                                           interval_ts*(num_invalid_samples-1);
      }
      else
      {
      ...
      MPU6050_MSG_4(
         MED, "%d invalid sample(s)sensor=%d last_ts=%u ts=%u end_ts=%u", num_invalid_samples,
         sensor_ptr->last_sample_timestamp,
         state->sensor_data.timestamp, state->sensor_data.end_timestamp);
      mpu6515_send_fifo_data(state, dmp_sensor, sensor_ptr, 0,  num_invalid_samples );
      sensor_ptr->last_sample_timestamp = state->sensor_data.end_timestamp;
    }

    if( fifo_info_ptr->num_pkts-num_invalid_samples )
    {
      state->sensor_data.status = SNS_DDF_SUCCESS;
      state->sensor_data.num_samples = (fifo_info_ptr->num_pkts-num_invalid_samples)*
                                       dmp_sensor_map[dmp_sensor].dim;
      if ( (fifo_info_ptr->num_pkts > 1) && ((fifo_info_ptr->num_pkts-num_invalid_samples)>1) )
      {
        state->sensor_data.timestamp = sensor_ptr->last_sample_timestamp + interval_ts;
      }
      else
      ...
      if( (fifo_info_ptr->num_pkts-num_invalid_samples)>5 )
      {
        MPU6050_MSG_4(
         MED, "%d valid samples last_ts=%u ts=%u end_ts=%u", fifo_info_ptr->num_pkts-num_invalid_samples,
         sensor_ptr->last_sample_timestamp,
         state->sensor_data.timestamp, state->sensor_data.end_timestamp);
      }
      mpu6515_send_fifo_data(state, dmp_sensor,sensor_ptr, num_invalid_samples,  fifo_info_ptr->num_pkts-num_invalid_samples );
      sensor_ptr->last_sample_timestamp = timestamp;
    }
  } // for (dmp_sensor=0; dmp_sensor<ARR_SIZE(parsed_fifo_info); dmp_sensor++)
}
...
static uint16_t mpu6515_parse_fifo_data(
    mpu6515_s*     state,
    sns_ddf_time_t timestamp,
    sns_ddf_time_t read_timestamp
)
{
  uint8_t dmp_sensor, dmp_pkt_size;
  uint16_t fifo_idx = 0;
  uint16_t total_pkts_parsed = 0;
  ...
  mpu6515_consume_fifo_data(state, timestamp, read_timestamp, dmp_sensor);
  dev_fifo_len -= fifo_idx;

  return total_pkts_parsed;
}
        ...
        sns_ddf_time_t    timestamp)
{
  ...
  uint8_t buffer = 0;
  uint8_t read_count;
  sns_ddf_time_t read_timestamp = 0;
  ...
  if(buffer & DMP_DATA_RDY_INT_ENABLED)
  {
    ...
    read_timestamp = sns_ddf_get_timestamp();
    bytes_read = sns_dd_dmp_read_fifo(state, DMP_FIFO_SIZE-dev_fifo_len, 0, &dev_fifo[dev_fifo_len]);
    if(bytes_read > 0)
    {
      dev_fifo_len += bytes_read;
      mpu6515_parse_fifo_data(state, timestamp, read_timestamp);
    }
  } // if(DMP interrupt)
}
                  ...
                  bool trigger_now )
{
  ...
  for ( i=0; i< MPU6515_MAX_FIFO_FLUSH_CYCLES; i++ )
  {
    timestamp  = sns_ddf_get_timestamp();
    bytes_read = sns_dd_dmp_read_fifo(state, max_num_bytes-dev_fifo_len,
                               MPU6515_SMALLEST_PKT_SIZE_BYTES, &dev_fifo[dev_fifo_len]);
    if ( bytes_read > 0 )
    {
      dev_fifo_len += bytes_read;
      MPU6050_MSG_1(LOW, "trigger_fifo_data - %d bytes read from FIFO", bytes_read);
      if ( mpu6515_parse_fifo_data(state, timestamp, timestamp) <= 1 )
      {
        break; //no pkts to read
      }
    }
    else
    {
      break; //no bytes to read fifo is empty
    }
  }
  return SNS_DDF_SUCCESS;
}
