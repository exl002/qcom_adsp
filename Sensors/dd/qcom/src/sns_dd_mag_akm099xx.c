...
$Id: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/dd/qcom/src/sns_dd_mag_akm099xx.c#1 $
    ...
    sns_dd_mag_akm099xx_state_s *state)
{
  ...
  {
    /* Returning the NULL to SMGR indicating no memory
     * even for basic err notify */
    sns_dd_mag099xx_notify_smgr(state, NULL, 0);
    /* Reset current state */
    state->curr_state = SNSD_MAG_ST_IDLE;
    return;
  }
