...
/*==============================================================================
  ...
  ---------- ---  -----------------------------------------------------------
  2014-08-07  sd  Updated MAG supported ODR list
  2014-06-26  sd  Updated get FIFO attribute
  2014-05-23  sd  Added 60hz to Mag supported odr list
  2014-04-15 lka  Added ODR=25Hz as a supported ODR. Fixed NV param reading.
    ...
    mpu6515_sensor_s*     sensor)
{
  ...
  MPU6050_MSG_2(MED, "nv_params: ddf_sensor=%u &nv_param=%u",
                sensor->sensor_type, entire_nv_params);
  ...
  else
  {
    ...
    MPU6050_MSG_5(MED, "nv_params: grp %u id %u axis map %d, %d, %d",
                  entire_nv_params->nvitem_grp, entire_nv_params->nvitem_id,
                  axes_map->indx_x * axes_map->sign_x,
                  axes_map->indx_y * axes_map->sign_y,
                  axes_map->indx_z * axes_map->sign_z);
  }
