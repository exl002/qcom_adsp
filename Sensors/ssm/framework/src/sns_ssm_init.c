...
/* $Header: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/ssm/framework/src/sns_ssm_init.c#2 $  */
/* $DateTime: 2014/06/02 15:32:27 $ */
/* $Author: pwbldsvc $ */
...
static sns_ssm_type_s sns_ssm;               // Primary data structure used by 
                                             // SSM framework
...
void sns_ssm_task(sns_ssm_type_s *sns_ssm)
{
   ...
   while(true)
   {
      ...
      OS_FLAGS    wait_type = 0;
      uint8_t     err;


      wait_type = SNS_SSM_QMI_WAIT_SIG;
      ...
      sig_flags = sns_os_sigs_pend(sns_ssm->ssm_flag_grp,
                                   ...
                                   0,
                                   &err);
      SNS_SSM_PRINTF1(LOW, "SSM: Received Signal %X", sig_flags);
      if(SSM_BIT_TEST(sig_flags, SNS_SSM_QMI_WAIT_SIG))
      {
         SNS_SSM_PRINTF0(LOW, "SSM: QMI message received");
