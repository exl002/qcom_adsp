...
/* $Header: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/sam/framework/src/8974/sns_sam_dsps_tgt.c#2 $ */
/* $DateTime: 2014/06/06 13:22:05 $ */
/* $Author: pwbldsvc $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2014-06-05  lka  Switches to buffering request if MD interrupt request fails.
  2014-03-14   ad  Fix crash due to unexpected deregistration
   ...
   bool addClientReq)
{
   ...
   if (sendPowerVote)
   {
      ...
      sns_pm_active_pwr_st_change_req_msg_v01 *msgPtr = 
         (sns_pm_active_pwr_st_change_req_msg_v01*)sns_smr_msg_alloc(SNS_DBG_MOD_DSPS_SAM,msgSize);
   ...
   bool enableInt)
{
   ...
   msgPtr = (sns_smgr_reg_hw_md_int_req_msg_v01*)sns_smr_msg_alloc(SNS_DBG_MOD_DSPS_SAM,msgSize);
   SNS_ASSERT(msgPtr != NULL);
   ...
   for(r = 0; r < SNS_SAM_QMD_MAX_NUM_DATA_REQ; r++)
   {
      if(sns_sam_mot_det[r].dataReqId == respPtr->ReportId)
      {
         if (respPtr->Resp.sns_result_t == SNS_RESULT_SUCCESS_V01)
         {
            ...
            if(sns_sam_qmd_motint_state == SNS_SAM_MOTION_INT_DISABLE)
            {
               err = sns_sam_update_smgr_req(sns_sam_mot_det[r].algoSvcId,
                                             sns_sam_mot_det[r].dataReqId,
   ...
   const void *smgrIndPtr)
{
   ...
   for(r = 0; r < SNS_SAM_QMD_MAX_NUM_DATA_REQ; r++)
   {
      if(sns_sam_mot_det[r].dataReqId == indPtr->ReportId)
      {
         ...
         else if(sns_sam_qmd_motint_state == SNS_SAM_MOTION_INT_ENABLE)
         {
            err = sns_sam_update_smgr_req(sns_sam_mot_det[r].algoSvcId,
                                          sns_sam_mot_det[r].dataReqId,
                                          SNS_SMGR_REPORT_REQ_V01);
         }
         else if(sns_sam_qmd_motint_state == SNS_SAM_MOTION_INT_DISABLE)
         {
            err = sns_sam_update_smgr_req(sns_sam_mot_det[r].algoSvcId,
                                          sns_sam_mot_det[r].dataReqId,
                                          SNS_SMGR_BUFFERING_REQ_V01);
   ...
   uint8_t dataReqId)
{
   ...
   if( sns_sam_qmd_motint_client_count == 0 )
   {
      SNS_SAM_PRINTF0(HIGH, "SAM: No MD clients to deregister");
      return;
   }
   ...
   bool addClient)
{
   ...
   else if (sns_sam_qmd_non_motint_client_count > 0)
   {
      sns_sam_qmd_non_motint_client_count--;
      if (sns_sam_qmd_non_motint_client_count == 0)
      {
