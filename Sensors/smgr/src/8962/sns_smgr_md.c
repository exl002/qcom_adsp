...
/* $Header: //components/rel/ssc.adsp/2.4.1/adsp_proc/Sensors/smgr/src/8962/sns_smgr_md.c#1 $ */
...
static bool sns_smgr_is_external_client(smgr_rpt_spec_s *rpt_p);
 ...
 */
/*=========================================================================*/
static void sns_smgr_de_suspend_one_rpt_request(uint8_t rpt_id,
                                                void *connection_handle)
{
  ...
  SMGR_FOR_EACH_Q_ITEM(&sns_smgr.report_queue, rpt_spec_ptr, rpt_link)
  {
    if ( (rpt_spec_ptr->rpt_id == rpt_id) &&
         (rpt_spec_ptr->header_abstract.connection_handle==connection_handle) &&
...
static boolean sns_smgr_set_hw_md_int(boolean enable_flag)
{
  ...
  if ( (enable_flag != sns_smgr.md.is_md_int_enabled) && (NULL != sensor_ptr) )
  {
    sns_ddf_status_e status = sns_smgr_set_attr(sensor_ptr, SNS_DDF_SENSOR_ACCEL,
                                                ...
                                                &enable_flag);
    /* disabling shall return SUCCESS */
    SNS_ASSERT((SNS_DDF_SUCCESS == status) || (FALSE!=enable_flag));
    ...
    if ( SNS_DDF_SUCCESS == status )
    {
      ...
      else
      {
        ...
        {
          sns_ddf_powerstate_e set_data = SNS_DDF_POWERSTATE_LOWPOWER;
...
{
  /*inital default by Power Manager Module is SNS_PM_ST_ACTIVE_LOW_V01 */
  sns_smgr.last_power_vote = SNS_PM_ST_ACTIVE_LOW_V01;
}
...
static void sns_smgr_check_rpts_coupled_with_md(uint8_t* power_vote_ptr)
{
  ...
  SMGR_FOR_EACH_Q_ITEM(&sns_smgr.report_queue, rpt_ptr, rpt_link)
  {
    ...
    for ( i=0; i<rpt_ptr->num_items; i++ )
    {
           ...
           (*power_vote_ptr != SNS_PM_ST_ACTIVE_HIGH_V01) )
      {
        ...
        *power_vote_ptr =  SNS_PM_ST_ACTIVE_HIGH_V01;
#else
        if ( (sns_smgr_is_external_client(rpt_ptr) != FALSE) ||
  ...
  SNS_SMGR_PRINTF2(LOW, "check_rpts_coupled_w_md - accel_w_md=%d non_accel=%d",
                   sns_smgr.md.num_accel_rpt_not_coupled_with_md,
                   sns_smgr.md.num_non_accel_items);
}
                                   ...
                                   sns_smgr_reg_hw_md_int_resp_msg_v01 *Resp_p )
{
  ...
  {
     sns_smgr_del_md_client(Msg_p->ReportId, Hdr_p->connection_handle);
     sns_smgr_de_suspend_one_rpt_request(Msg_p->ReportId, Hdr_p->connection_handle);
...
/*===========================================================================
  ...
  FUNCTION:   sns_smgr_is_external_client

===========================================================================*/
/*!
  @brief check if this rpt is from external client

  @detail

  @param  [in] rpt_p, report pointer;
  @return true:  report is from external client
          false: report is from internal client
 */
/*=========================================================================*/
static bool sns_smgr_is_external_client(smgr_rpt_spec_s *rpt_p)
{
  uint8_t   src_module=rpt_p->header_abstract.src_module;
#if defined(SNS_PCSIM)
  if ( ( src_module< SNS_MODULE_DSPS_IST) || (src_module > SNS_MODULE_DSPS_SCM ))
  {
     return true;
  }
  return false;
#else
  return (!SNS_SMR_IS_INTERNAL_MODULE(src_module)) ;
#endif
}
...
{
  uint8_t new_vote        = SNS_PM_ST_ACTIVE_LOW_V01;
  uint8_t old_num_reports = sns_smgr.md.total_rpt_num;
  smgr_power_state_type_e power_rail = sns_hw_powerrail_status();

  sns_smgr_check_rpts_coupled_with_md(&new_vote);
  sns_smgr.md.total_rpt_num = sns_q_cnt(&sns_smgr.report_queue);

#ifndef SNS_EXCLUDE_POWER
  SNS_SMGR_PRINTF2(MED, "check_rpts_4_md_update - old_num_reports=%d new_num_rpts=%d",
                   old_num_reports, sns_smgr.md.total_rpt_num);
  if ( (sns_smgr.md.total_rpt_num != old_num_reports) &&
       (sns_smgr.md.total_rpt_num > 0) )
  {
    /* there is report now, enable power rail */
    if (sns_smgr.md.num_non_accel_items )
    {
      /* there are no accel report, put to high power  */
      power_rail = SNS_SMGR_POWER_HIGH;
    }
    else
    {
      /* there are only accel report, put to low power  */
      power_rail = SNS_SMGR_POWER_LOW;
    }
  }
  else if ( (sns_smgr.md.total_rpt_num == 0) &&
            (sns_smgr.sensor_test_info.test_done) )
  {
    /* no report now, put to power off */
