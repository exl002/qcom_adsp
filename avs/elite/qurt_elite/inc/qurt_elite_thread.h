...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/qurt_elite/inc/qurt_elite_thread.h#11 $
...
/** Maximum number of hardware threads. */
#define QURT_ELITE_MAX_HW_THREADS       3
...
/**
  Low power optimization feature flags.
 */
typedef enum
{
   QURT_ELITE_LP_TCB_OPT=1,  /**< TCB Optimization. */
}  QURT_ELITE_LP_OPT_FLAGS;  /**< Low power optimization feature flags. */
...
/**
Creates and launches a thread. Thread context buffer (TCB) will be
allocated in small TCB buffer (which can be OCMEM or LPI). If small
TCB buffer is full, then thread context buffer (TCB) will be allocated
in normal TCB buffer.

@param[out] pTid Thread ID.
@param[in] pzThreadName Thread name.
@param[in] pStack Location of a preallocated stack (NULL
...
@param[in] nPriority Thread priority, where 0 is the lowest and
                    255 is the highest.
@param[in] pfStartRoutine Entry function of the thread.
@param[in] arg This parameter is passed to the entry
              function and can be a case to any pointer type.
@param[in] heap_id Thread stack will be allocated in the heap
              specified by this parameter. If the heap specified by
              heap_id is full, thread stack will be allocated in default heap.  
@param[in] lp_opt_flags Flags indicating the power optimization features. 
              Each bit represent different optimization.
