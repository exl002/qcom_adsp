...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/qurt_elite/src/qurt_elite_thread.cpp#11 $
...
/*
 ...
 lp_opt_flags   : Flags indicating the power optimization features. Each bit represent different optimization.
                  bit (0) = 1 indicates thread TCB need to be created in small TCB buffer. 
                  bit (0) = 0 indicates thread TCB need to be created in regular TCB buffer. 
                  bit(31:1) reserved.
 return
 An indication of success or failure.
*/
static ADSPResult qurt_elite_thread_create(qurt_elite_thread_t *pTid, char *pzThreadName, char* pStack,
...
ADSPResult qurt_elite_thread_launch(qurt_elite_thread_t *pTid, char *pzThreadName, char* pStack,
   size_t nStackSize, int nPriority, int (*pfStartRoutine)(void *), void* arg, QURT_ELITE_HEAP_ID heap_id)
{
   uint32_t lp_opt_flags = 0;
   return (qurt_elite_thread_create(pTid, pzThreadName, pStack, nStackSize, nPriority, pfStartRoutine,
                                    arg, heap_id, lp_opt_flags));
}
   ...
   uint32_t lp_opt_flags)
{
   ...
   if (lp_opt_flags & QURT_ELITE_LP_TCB_OPT)
   {
      // TCB optimization is enabled in low power thread, create TCB in small TCB buffer. 
      MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "small TCB thread attempted, THRD: Name = %s", pzThreadName);   
      qurt_thread_attr_set_tcb_partition(&attr, 1);
   }
   
   ret  = __real_qurt_thread_create((qurt_thread_t*)pTid, &attr, qurt_elite_thread_stub, (void*)pThreadNode);

   if (QURT_EOK != ret)
   {
      if (lp_opt_flags & QURT_ELITE_LP_TCB_OPT)
      {
         // Thread create could fail if small TCB buffer is full. Attempt to allocate in normal TCB buffer. 
         // If there is error further, return error.	  
         MSG_1(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "small TCB thread failed, THRD: Name = %s", pzThreadName);      
	  
         //create TCB in large TCB buffer. 
      ...
      if (QURT_EOK != ret)
      {
         *pTid = ret;
         ...
         // If there is failure to create thread, clear thread state.
...
static void qurt_elite_thread_stub(void *arg)
{
   ...
   __real_qurt_thread_exit(exit_status);
...
void qurt_elite_thread_join(qurt_elite_thread_t nTid, int* nStatus)
{
   *nStatus = ADSP_EBADPARAM;
   ...
   (void) __real_qurt_thread_join(nTid, &dummy);
