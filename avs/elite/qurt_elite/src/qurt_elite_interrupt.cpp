...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/qurt_elite/src/qurt_elite_interrupt.cpp#11 $
...
ADSPResult qurt_elite_interrupt_register( qurt_elite_interrupt_ist_t *ist_ptr, uint16_t intr_id,
                              void (*callback_ptr)(void *),void *arg_ptr,char_t *thread_name)
{
   uint32_t result;
 ...
 //initialise the structure members
   ...
   if (ADSP_FAILED(result = qurt_elite_thread_launch(&(ist_ptr->thread_id), ist_ptr->ist_thread_name, NULL,
        1024, ELITETHREAD_DEFAULT_IST_PRIO, qurt_elite_interrupt_workloop, (void*)ist_ptr, QURT_ELITE_HEAP_DEFAULT)))
