...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/qurt_elite/src/qurt_elite_globalstate.cpp#21 $
...
{
   mem_wrap_init_heap_stats(&qurt_elite_globalstate.heap_stats);

   //initialize mem leak debug data here only if MALLOC_WRAPPER is 
   //not enabled. If enabled, the debug data will be initialized by the
   //wrappers themselves.
   ...
   qurt_elite_globalstate.ddr_strm_lpa_heap =  NULL;

   qurt_elite_globalstate.ddr_device_lpa_heap =  NULL;
...
void qurt_elite_globalstate_deinit(void)
{
   ...
   while (pClientNode) {
      ...
      while (pMemoryMapNode) {
         ...
         qurt_elite_memory_free(pMemoryMapNode);
         pMemoryMapNode = pClientNode->pMemMapListNode;
      }
   ...
   mem_wrap_deinit_heap_stats(&qurt_elite_globalstate.heap_stats);

#ifndef MALLOC_WRAPPER
   mem_wrap_deinit_memleak_data();
#endif //MALLOC_WRAPPER
...
ADSPResult lpm_init(void)
{
   ...
   qurt_mem_region_attr_set_mapping(&reg_attr, QURT_MEM_MAPPING_IDEMPOTENT);
   ...
   //Create the LPA stream heap from DDR
   glob_state->size_strm_lpa_heap_from_ddr = lpm_prop_ptr->size_strm_lpa_heap_from_ddr;
   if(0 != glob_state->size_strm_lpa_heap_from_ddr)
   {
      /* 4KB address alignment so that it can be placed in OCMEM */ 
      glob_state->ddr_strm_lpa_heap = (uint32_t*)qurt_elite_memory_aligned_malloc(glob_state->size_strm_lpa_heap_from_ddr, 4096, QURT_ELITE_HEAP_DEFAULT);
      if(NULL == glob_state->ddr_strm_lpa_heap)
      {
   ...
   //Create the LPA device heap from DDR
   glob_state->size_device_lpa_heap_from_ddr = lpm_prop_ptr->size_device_lpa_heap_from_ddr;
   if(0 != glob_state->size_device_lpa_heap_from_ddr)
   {
      /* 4KB address alignment so that it can be placed in OCMEM */
      glob_state->ddr_device_lpa_heap = (uint32_t*)qurt_elite_memory_aligned_malloc(glob_state->size_device_lpa_heap_from_ddr, 4096, QURT_ELITE_HEAP_DEFAULT);
      if(NULL == glob_state->ddr_device_lpa_heap)
      {
	     MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO," Failure in mallocing the LPA device heap from DDR!! Exiting..");
	     lpm_deinit();
	     return ADSP_EFAILED;
      }
   }
...
void lpm_deinit(void)
{
    ...
    if(NULL != qurt_elite_globalstate.ddr_strm_lpa_heap)
	{
		qurt_elite_memory_aligned_free(qurt_elite_globalstate.ddr_strm_lpa_heap);
		qurt_elite_globalstate.ddr_strm_lpa_heap = NULL;
	}
    if(NULL != qurt_elite_globalstate.ddr_device_lpa_heap)
	{
		qurt_elite_memory_aligned_free(qurt_elite_globalstate.ddr_device_lpa_heap);
		qurt_elite_globalstate.ddr_device_lpa_heap = NULL;
	}
}
