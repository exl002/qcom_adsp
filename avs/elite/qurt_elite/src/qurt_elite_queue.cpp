...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/qurt_elite/src/qurt_elite_queue.cpp#12 $
...
ADSPResult qurt_elite_queue_create(char *pzQ2Name, int nMaxItems, qurt_elite_queue_t** pQ)
{
   ...
   // initialize mutex
   qurt_elite_mutex_init(&((*pQ)->mqLock));
...
void qurt_elite_queue_destroy(qurt_elite_queue_t* pQ)
{
   ...
   qurt_elite_mutex_destroy(&pQ->mqLock);
...
ADSPResult qurt_elite_queue_push_back(qurt_elite_queue_t* pQ, uint64_t* pPayload)
{
   ...
   // grab the mutex
   qurt_elite_mutex_lock(&pQ->mqLock);

   if (TRUE == pQ->fDisableQ)
   {
    qurt_elite_mutex_unlock(&pQ->mqLock);
...
ADSPResult hexox_queue_disable(qurt_elite_queue_t* pQ)
{
    // check for NULL
