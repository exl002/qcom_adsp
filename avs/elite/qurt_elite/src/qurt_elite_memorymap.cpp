...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/qurt_elite/src/qurt_elite_memorymap.cpp#14 $
                    ...
                    uint32_t* shm_mem_map_handle_ptr)
{
    ...
    qurt_elite_memorymap_node_t* mem_map_node_ptr = (qurt_elite_memorymap_node_t*)(*shm_mem_map_handle_ptr);
    //set the mapping type as physical
    mem_map_node_ptr->mapping_mode = QURT_ELITE_MEMORYMAP_PHYSICAL_ADDR_MAPPING_ACCESS_OFFSET;

    //add this memory map to the client
    qurt_elite_memorymap_add_mem_map_node_to_client(client_token,mem_map_node_ptr);

    #ifdef DEBUG_QURT_ELITE_MEMORYMAP
    MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "qurt_elite_memorymap successfully mapped all regions(offset mode) and added to client's list (token,mmhandle,num of regions) = (0x%x,0x%x,%d)",
            (unsigned int)client_token, (unsigned int)mem_map_node_ptr, num_shm_reg);
    #endif
                    ...
                    uint32_t* shm_mem_map_handle_ptr)
{
    ...
    if (NULL == shm_mem_reg_ptr)
    {
        MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "qurt_elite_memorymap shm_mem_reg_ptr is null while mapping heap addr");
        return ADSP_EBADPARAM;
    }

    if (NULL == shm_mem_map_handle_ptr)
    {
        MSG(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "qurt_elite_memorymap shm_mem_map_handle_ptr is null while mapping heap addr");
        return ADSP_EBADPARAM;
    }
    ...
    *shm_mem_map_handle_ptr = 0;
    ...
    // for heap addr mapping, assumption is the memory is already mapped.
    // make sure heap addr of the region is mapped before and if not return an error.
    // Since it is heap addr, shm_mem_reg_ptr->shm_addr_lsw is non zero and shm_mem_reg_ptr->shm_addr_msw must be zero.
    // make sure the shm_mem_reg_ptr->shm_addr_lsw is 4 byte aligned and the shm_mem_reg_ptr->mem_size is 4 byte multiple.
    if ((shm_mem_reg_ptr->shm_addr_lsw & 0x3) || (shm_mem_reg_ptr->mem_size & 0x3))
    {
        MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
        "qurt_elite_memorymap heap addr mapping failed since the heap address or size is not multiple of 4 bytes (client token,heap addr,heap size) 0x%x, 0x%x, %d)",
        (unsigned int)client_token,(unsigned int)shm_mem_reg_ptr->shm_addr_lsw,(unsigned int)shm_mem_reg_ptr->mem_size);
        return ADSP_EFAILED;
    }

    if ((0 != shm_mem_reg_ptr->shm_addr_msw) ||
        (0 == qurt_elite_memorymap_get_physical_addr(shm_mem_reg_ptr->shm_addr_lsw)) ||
        (0 == qurt_elite_memorymap_get_physical_addr(shm_mem_reg_ptr->shm_addr_lsw + shm_mem_reg_ptr->mem_size -1))
       )
    {
        MSG_3(MSG_SSID_QDSP6, DBG_ERROR_PRIO,
        "qurt_elite_memorymap heap addr mapping failed since the corresponding phy cannot be found (client token,heap addr,heap size) 0x%x, 0x%x, %d)",
        (unsigned int)client_token,(unsigned int)shm_mem_reg_ptr->shm_addr_lsw, (unsigned int)shm_mem_reg_ptr->mem_size);
        return ADSP_EFAILED;
    }
    ...
    // Allocate a node for this mapping (the global linklist for a client's mapped regions is
    // organized as one-node-per-invocation.
    // Pointer to the phys region record in the node.
    qurt_elite_memorymap_region_record_t * cont_phys_region_ptr;

    // Determine the size of the node, size of one memmap node + one memmap region.
    uint32_t total_node_alloc_size = sizeof(qurt_elite_memorymap_node_t) + sizeof(qurt_elite_memorymap_region_record_t);
        ...
        QURT_ELITE_HEAP_DEFAULT)) ))
    {
        ...
        // Initialize the node
        memset(mem_map_node_ptr, 0, total_node_alloc_size);
        mem_map_node_ptr->unNumContPhysReg = 1;

        cont_phys_region_ptr = (qurt_elite_memorymap_region_record_t*) ((uint8_t*)mem_map_node_ptr + sizeof(qurt_elite_memorymap_node_t));
        // Copy over the phys addr and size to the record in the node.
        cont_phys_region_ptr->shm_addr.mem_addr_64b = qurt_elite_memorymap_get_physical_addr(shm_mem_reg_ptr->shm_addr_lsw);
        // in this case shm_addr_lsw is the virt addr
        cont_phys_region_ptr->virt_addr = shm_mem_reg_ptr->shm_addr_lsw;
        cont_phys_region_ptr->mem_size = shm_mem_reg_ptr->mem_size;
#ifdef DEBUG_QURT_ELITE_MEMORYMAP
        MSG_4(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "qurt_elite_memorymap node, successfully mapped  mem_map_node_ptr(%#x), phy addr lsw (%#x), phy addr msw (%#x), virt addr(%#x)", (unsigned int)mem_map_node_ptr, (unsigned int)cont_phys_region_ptr->shm_addr.mem_addr_32b.lsw, (unsigned int)cont_phys_region_ptr->shm_addr.mem_addr_32b.msw, (unsigned int)cont_phys_region_ptr->virt_addr);
#endif
        }
    ...
    *shm_mem_map_handle_ptr = (uint32_t)mem_map_node_ptr;
    ...
    //set the mapping type as heap addr mapping
    mem_map_node_ptr->mapping_mode = QURT_ELITE_MEMORYMAP_HEAP_ADDR_MAPPING;

    //add this memory map to the client
    qurt_elite_memorymap_add_mem_map_node_to_client(client_token,mem_map_node_ptr);

    #ifdef DEBUG_QURT_ELITE_MEMORYMAP
    MSG_2(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "qurt_elite_memorymap successfully mapped heap region and added to client's list (token,mmhandle) = (0x%x,0x%x)",
          (unsigned int)client_token, (unsigned int)mem_map_node_ptr);
    #endif
...
static ADSPResult qurt_elite_memorymap_region_create(
    ...
    case CREATE_MASTER_REGION:
        ...
        qurt_mem_region_attr_set_physaddr(&memreg_attr, 0xFFFFFFFF);
        break;
    case MAP_PHYSICAL_REGION_TO_VIRTUAL_FIXED:
        ...
        qurt_mem_region_attr_set_physaddr(&memreg_attr, reg_ptr->shm_addr.mem_addr_32b.lsw );
...
static ADSPResult qurt_elite_memorymap_region_delete(qurt_elite_memorymap_node_t *mem_map_node_ptr)
{
    ...
    if ((QURT_ELITE_MEMORYMAP_PHYSICAL_ADDR_MAPPING == mem_map_node_ptr->mapping_mode) ||
        (QURT_ELITE_MEMORYMAP_PHYSICAL_ADDR_MAPPING_ACCESS_OFFSET == mem_map_node_ptr->mapping_mode) ) {
        // Bookmark the first phys region:
                                ...
                                )
{
    ...
    bool_t local_get_reg_attrib_flag = get_reg_attrib_flag;
    ...
    if (QURT_ELITE_MEMORYMAP_PHYSICAL_ADDR_MAPPING_ACCESS_OFFSET == mem_map_node_ptr->mapping_mode)
    {   //In this case it should be interpreted as offset from the virtual base address
        uint32_t offset = (uint32_t)phy_addr_64bits;
        uint32_t mapped_region_size = cont_phy_regions_ptr[mem_map_node_ptr->unNumContPhysReg].mem_size;

        if (offset >= mapped_region_size)
        {
            MSG_2(MSG_SSID_QDSP6, DBG_ERROR_PRIO, "qurt_elite_memorymap get virt_addr offset is outsite the mapped region(offset,mapped_region_size)->(0x%x,0x%x)",
                    (unsigned int)offset,(unsigned int)mapped_region_size);
            req_virt_addr = 0;
            return req_virt_addr;
        }
        else
        {
            req_virt_addr = (cont_phy_regions_ptr[mem_map_node_ptr->unNumContPhysReg].virt_addr + offset );
        }

        if (TRUE == local_get_reg_attrib_flag)
        {
            //overwrite the phy_addr_64bits with the corresponding phy addr
            phy_addr_64bits = qurt_elite_memorymap_get_physical_addr(req_virt_addr);
        }

    }
    else
    {
        //for virtual or heap mapping mode, phy_addr_64bits_inp is interpreted as virtual address. Hence convert
        ...
        if ((QURT_ELITE_MEMORYMAP_VIRTUAL_ADDR_MAPPING == mem_map_node_ptr->mapping_mode) ||
            (QURT_ELITE_MEMORYMAP_HEAP_ADDR_MAPPING == mem_map_node_ptr->mapping_mode))
        {
            phy_addr_64bits = qurt_elite_memorymap_get_physical_addr(uint32_t(phy_addr_64bits_inp & 0xffffffff));
        }

        //For other memory mapping modes, get the memory attributes by default.
        local_get_reg_attrib_flag = TRUE;
    ...
    //For  QURT_ELITE_MEMORYMAP_PHYSICAL_ADDR_MAPPING_ACCESS_OFFSET, search need not be performed unless explicitly requested by (get_reg_attrib_flag) flag.
    //For other modes, the search must be performed by default.
    ...
    if (TRUE == local_get_reg_attrib_flag)
    {
        ...
        while(left <= right) {
            ...
            if((phy_addr_64bits >= cont_phy_regions_ptr[middle].shm_addr.mem_addr_64b) &&
               (phy_addr_64bits < cont_phy_regions_ptr[middle].shm_addr.mem_addr_64b + (uint64_t)cont_phy_regions_ptr[middle].mem_size) ) {
                found = TRUE;
                break;
            } else {
                if(phy_addr_64bits < cont_phy_regions_ptr[middle].shm_addr.mem_addr_64b) right = middle - 1;
                if(phy_addr_64bits >= cont_phy_regions_ptr[middle].shm_addr.mem_addr_64b + (uint64_t)cont_phy_regions_ptr[middle].mem_size) left = middle + 1;
            }
        }
                    ...
                    uint32_t mem_size)
{
    ...
    if(ADSP_EOK != (rc = qurt_elite_memorymap_get_mapping_mode(client_token,
                             shm_mem_map_handle, &mapping_mode)))
    {
        return rc;
    }
    ...
    rc = qurt_elite_memorymap_cache_flush(virt_addr, mem_size);
                    ...
                    uint32_t mem_size)
{
    ...
    rc = qurt_elite_memorymap_cache_invalidate(virt_addr, mem_size);
...
static ADSPResult qurt_elite_memorymap_verify_pool_range(uint32_t shm_addr,uint32_t mem_size, QURT_ELITE_MEMORYPOOLTYPE *pool_id_ptr)
{
    ...
    case QURT_ELITE_MEMORYMAP_EBI1_POOL:
        ...
        if ( (shm_addr < (uint32_t) EBI_START
             || (shm_addr + mem_size) > (uint32_t)(EBI_START + EBI_SIZE)))
        {
            rc = ADSP_EFAILED;
        }
        break;
    case QURT_ELITE_MEMORYMAP_SMI_POOL:
        if ( (shm_addr < (uint32_t) SMI_POOL_BASE
             || (shm_addr + mem_size) > (uint32_t) (SMI_POOL_BASE + SMI_POOL_SIZE)))
        {
            rc = ADSP_EFAILED;
        }
        break;
