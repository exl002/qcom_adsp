...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/elite/utils/inc/EliteLoggingUtils_i.h#10 $
...
@{ */
/**
  Maximum size of the log packet. The maximum packet size that can be allocated
  is 6176 bytes.
  ...
  In any case, the log buffer size must never be more than 6176 bytes per log 
  packet
*/
...
@{ */
/** Logging utility user's PCM data information.
*/
...
/** Logging utility user's log header and data payload information.
*/
struct elite_log_info
{
    elite_qxdm_log_code         qxdm_log_code;
    /**< QXDM log code for the log packet.
...
/** @ingroup elitelog_func_alloc_buff
  ...
  @return
  Pointer to the allocated log packet. Returns NULL if buffer allocation fails or
  log code is disabled.
...
/** @ingroup elitelog_func_log_buff
  ...
  @param[in] log_info_ptr  Pointer to the object containing the logging utility
                           user's log header and data payload information.

  @return
  ADSP_EOK if success or an error code for any failure.
...
/** @ingroup elitelog_func_alloc_buf_log_data
  ...
  @param[in] log_info_ptr  Pointer to the object containing the logging utility
                           client's log header and data payload information.
