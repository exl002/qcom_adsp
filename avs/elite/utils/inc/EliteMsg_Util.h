...
--------   ---     -------------------------------------------------------
03/20/13   sw      (Tech Pubs) Updated Doxygen comments and markup for 2.2
...
/** @ingroup elitemsg_func_finish
  Called after a message has been processed and is ready to be released or
  acknowledged back.

  @datatypes
  elite_msg_any_t

  @param[in] pAnyMsg            Pointer to the message that has been processed.
  @param[in] unResponseResult   Fills the response result if the message must
...
/** @ingroup elitemsg_func_return_payld_buf
  Returns the payload buffer to the designated buffer queue when given a message
  with a pointer to the payload buffer. For efficiency, the caller ensures that
  the BufferReturnQ always exists in the payload header.
...
/** @ingroup elitemsg_func_push_payld
  ...
  This function directly pushes any payloads to its return queue. It can be
