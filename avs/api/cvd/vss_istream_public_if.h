...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/api/cvd/vss_istream_public_if.h#10 $
  $Author: coresvc $
...
/**
  ...
  1. The encoder and decoder buffers provided must be large enough to acommodate
     the maximum possibble vocoder packet size.
...
{
  uint32_t mem_handle;
    /**< Handle to the shared memory that holds endcoder and decoder buffers.
  ...
  uint32_t dec_buf_size;
    /**< Size of the decoder buffer. The amount of memory allocated
         at dec_buf_size must meet the requirement as specified in @latexonly
         \hyperref[shmem_guideline]{Shared Memory Usage Guideline} 
         @endlatexonly. */
}
#include "apr_pack_end.h"
...
/**
  ...
  the the client in response to the previous #VSS_ISTREAM_EVT_OOB_NOTIFY_ENC_BUFFER_READY
  event by the time a new encoder packet is ready for delivery, the stream will drop
  the new encoder packet.
*/
