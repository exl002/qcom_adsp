...
$Header: //source/qcom/qct/images/adsp/avs/rel/2.2/adsp_proc/avs/api/audio/inc/adsp_adm_api.h#16 $
...
/** @ingroup admsvc_cmd_map_shared_mem
  ...
  @apr_msgpayload{avs_cmd_shared_mem_map_regions_t}
    Following this structure are num_regions of the
    @xhyperref{hdr:AdmMapPayload,Map payload}.
    @tablens{weak__avs__cmd__shared__mem__map__regions__t}

  @mappayload{avs_shared_map_region_payload_t}
    @xreflabel{hdr:AdmMapPayload}
...
/** Immediately following this structure are num_sessions of the <b>Session
    map node payload</b> (see below).
*/
...
/** @} */ /* end_addtogroup admsvc_cmd_device_open_v5 */
...
/** @ingroup admsvc_cmd_close_device
  ...
  @apr_hdr_fields
    ...
    @par
    The client must set the COPP ID for the LPCM use case, and the Device ID
    for the Compressed use case

  @apr_msgpayload
    None.

  @detdesc
    In the Linear Pulse Code Modulation (LPCM) use case (mode_of_operation =
    1-4), this command closes the COPP and disconnects the device session.
    @par
    In the Compressed use case (mode_of_operation = 5-6), this command
...
/** @ingroup admsvc_cmd_set_pp_params
  ...
  @par Parameter data payload (adm_param_data_v5_t)
...
/** If the data payload addrress is NULL, this structure is immediately
    followed by a series of structures containing the pre/postprocessing
    parameter data. See <b>Parameter data payload</b> below.
*/
...
/** @ingroup admsvc_resp_get_pp_param_values
...
/** Immediately following this structure is the acknowledgement <b>Parameter
    data variable payload</b> containing the pre/postprocessing parameter 
    data. For an in-band scenario, the variable payload depends on the size
    of the parameter.
*/
...
/** Allows a client to control the gains on various session-to-COPP paths.
  ...
  @par Session COPP gain type (weak_adm_session_copp_gain_v5_t)
  ...
  @detdesc
    For every session-to-COPP path, the gains specified are channel specific
    and can be applied immediately or achieved over a period of time in
    ...
    to indicate the kind of ramping required to achieve the target gain. This
    command allows clients to specify gain ramps for multiple session-to-COPP
    paths at one time, allowing better synchronization. It can also be used
    to apply the gain ramp globally to all session-to-COPP paths that exist
    ...
    @par
    It is the client's responsibility to maintain a history of the gain values
    for every channel in each of the session-to-COPP paths because this command
    ...
    If the client wants to change the gains selectively of only some of the
    ...
    mapping information (i.e., from ADM_CMD_MULTICHANNEL_COPP_OPEN) to
    determine which gains to use from this command. Therefore, it is the
    client's responsibility to send this command with gain values set for
    exactly those channels on which the device is operating. For example:
    @par
      - If the audio stream is multichannel, the user sets a multichannel
        ...
        mode, the client converts the multichannel gain to stereo gain and sets
        gains for only two channels in this command. The aDSP ignores the
      ...
      - If the audio stream is stereo, the user sets a stereo volume, and the
        ...
        all six channels in this command. @newpage
...
/* Payload of the ADM_CMD_MATRIX_RAMP_GAINS_V5 command, which allows a client
    ...
    paths. */
/** Immediately following this structure are num_gains of the <b>Session COPP
    gain type</b> (see below). 
*/
...
/** Allows a client to set mute/unmute on various session-to-COPP paths.
  ...
  @apr_msgpayload{adm_cmd_matrix_mute_v5_t}
    @table{weak__adm__cmd__matrix__mute__v5__t} @newpage
  ...
  @detdesc
    ...
    @par
      ...
      - Overrides any #ADM_CMD_MATRIX_RAMP_GAINS_V5 that may be called
    @par
...
struct adm_cmd_connect_afe_port_v5_t
{
    ...
    /* Q6 = Hex MM ADSP.BF.2.0: AFE Interface Specification (TBD) */
...
/** @ingroup admsvc_cmd_add_topos
  ...
  @par Buffer format (audproc_custom_topologies_t)
    ...
    @xhyperref{hdr:AdmTopoDefinition,Topology definition}).
  ...
  @keep{8} @par Topology definition (audproc_topology_definition_t)
    @xreflabel{hdr:AdmTopoDefinition}
    Immediately following this structure are num_modules of
    @xhyperref{hdr:AdmModuleInfo,Module information}.
    @tablens{weak__audproc__topology__definition__t}

  @par Module information (audproc_module_info_t)
    @xreflabel{hdr:AdmModuleInfo}
  ...
  @detdesc 
    ...
    the ADM supports up to a total of 128 custom topologies. If a subsequent
    command redefines a previously defined topology ID, the previous topology
    definition is replaced with the new one. However, the changes take effect
    only on the next session creation. @vertspace{-6}

  @return
    APRV2_IBASIC_RSP_RESULT (refer to @xhyperref{Q3,[Q3]}).
    The handle is defined by the client as input. @vertspace{-6}
...
@{ */
/** Provides updates to the matrix/stream router mapping between session IDs and device IDs.
  ...
  @par Session map node payload (adm_stream_device_map_node_v5_t)
...
{
    uint32_t                  mode;
    /**< specifies the mode of opertation of the particular session
...
/** Immediately following this structure are num_devices entries of device end point IDs.
    The device end point IDs are 16 bits, so there is another 16-bit field for padding if
    num_devices is an odd number. For PCM devices, this is the COPP Id. For compressed
    audio devices this is the Id that was returned in ADM_CMDRSP_DEVICE_OPEN_V5.
*/
...
/** @ingroup admsvc_cmd_strm_dev_unmap_rout
  ...
  @par Session map node payload (adm_stream_device_unmap_node_v5_t)
...
/** Immediately following this structure are num_sessions of the <b>stream/device
    unmap node payload</b> (see below).
*/
struct adm_cmd_stream_device_unmap_routings_v5_t
{
    uint32_t                  mode;
    /**< specifies the mode of operation of the particular session
...
#include "adsp_end_pack.h"
;
/** @} */ /* end_weakgroup weak_adm_session_unmap_node_v5_t */
/** @endcond */
...
/** @addtogroup audio_matrix_stream_router_chmixer
@{ */
/** Must be used for default channel-mixer related parameters.
...
/** Used by #MTMX_MODULE_ID_DEFAULT_CHMIXER to configure the channel mixer
  ...
  @par Rules for using Set parameters
    ...
    - The Set parameters values are lost once the stream/device is closed. @newpage
    ...
    - The aDSP reverts to default coefficients (built-in coefficients) if the
      rule is not explicitly provided by the client; e.g., if the input or output
      media types do not match the Set parameter rule provided.
...
/** @} */ /* end_addtogroup audio_matrix_stream_router_chmixer */

typedef struct default_chmixer_param_id_coeff default_chmixer_param_id_coeff;

#include "adsp_begin_pack.h"

/** @addtogroup weak_default_chmixer_param_id_coeff
@{ */
