	...
	$targetName=$ARGV[1];
	open(LOGFILE, ">$inputfileName\\InternalLog\.txt") or die "not able to open file \"InternalLog.txt\" in write mode";#Writing to a file "system.xls" which is output.
	open(DUMPFILE, ">$inputfileName\\DSPAnalysis\.txt") or die "not able to open file \"DSPAnalysis.txt\" in write mode";#Writing to a file "system.xls" which is output.
	#$TCBPointer="F05E5080";
	$noissueFlag = 0;
...
$heapstatsFlag =0;
if (($targetName =~ /8x26/) || ($targetName =~ /8974/) || ($targetName =~ /8626/))
{
