...
NOTE: The @brief description above does not appear in the PDF.
      ...
      The Elite_CAPI_mainpage.dox file contains the group/module descriptions that
      are displayed in the output PDF generated using Doxygen and LaTeX. To
      edit or update any of the group/module text in the PDF, edit the
      Elite_CAPI_mainpage.dox file or contact Tech Pubs.
===========================================================================*/
...
typedef enum
{
  ...
  DEC_NEED_MORE=0x1000,        /**< More input data is required to decode the
                                    frame. @xreflabel{hdr:DecNeedMoreError} */
  DEC_RECOVERY_SUCCESS,        /**< Header parsing failed; the next
                                    synchronization word is found in the
                                    bitstream.*/
  DEC_RECOVERY_FAILURE,        /**< Tried to recover; no synchronization word
                                    is found in the input buffer. */
  DEC_BADPARAM_FAILURE,        /**< Invalid parameters detected during
                                    decoding. */
  DEC_BUFFERTOOSMALL_FAILURE,  /**< Output buffer is too small and cannot hold
                                    a frame. @xreflabel{hdr:DecBufSmallError} */
  ...
  DEC_FAILURE,                 /**< All other CAPI non-specific failures.
                                    The complete buffer is discarded. No output
                                    is produced in this call. */
...
typedef enum
{
  ...
  ENC_NEED_MORE=0x2000,        /**< More input data is required to encode the
                                    frame. @xreflabel{hdr:EncNeedMoreError} */
  ENC_BADPARAM_FAILURE,        /**< Invalid parameters detected during
                                    encoding. */
  ENC_BUFFERTOOSMALL_FAILURE,  /**< Output buffer is too small and cannot hold
                                    a frame. @xreflabel{hdr:EncBufSmallError} */
...
/** @xreflabel{hdr:CommonCapiParamsEnum}
    ...
    are to be used with the CAPI GetParam() and SetParam() routines.
*/
typedef enum
{
   eIcapiNumChannels=0x0,
   /**< For decoders, used with a GetParam() call to get the number of channels
        in the last decoded output buffer. If this parameter is queried before
        decoding any frame, it returns 0. For encoders, it used with a
        SetParam() call to set the number of channels in the latest raw input
        buffer. */

   eIcapiSamplingRate,
   /**< For decoders, used with a GetParam() call to get the sampling rate (in
        Hertz) of the last decoded output buffer. For encoders, it is used
        with a SetParam() call to convey the sampling rate of the latest raw
        input buffer. */
   ...
   /**< Used with the GetParam() function to get the maximum size of metadata that
        the CAPI will return. Zero indicates that the CAPI does not support
        metadata. */

   eIcapiMetaDataPtr,
   /**< Used with the GetParam() call to query the pointer to the metadata.

        The CAPI packs metadata as groups of asm_stream_param_data_t
        ...
        @indent{12} First asm_stream_param_data_t \n
        @indent{12} First metadata \n
        @indent{12} Second asm_stream_param_data_t \n
        @indent{12} Second metadata \n
        @indent{12} and so on */
...
struct ICAPIVtbl
{
   ...
   /** @ingroup capi_virt_func_set_param
       ...
       - #eIcapiMetaDataPtr
       @par
       If a new index needs to be declared, it must be present in the
   ...
   /** @ingroup capi_virt_func_get_param
       ...
       @par
       For nParamIdx, choose one of the common enumerated indices defined in
       #CommonCAPIParams. If a new index needs to be declared, it must be
   ...
   /** @ingroup capi_virt_func_end
       Returns the library to the unitialized state.
...
/** @ingroup function_wrappers
    ...
    compilation, the CAPI interface is simply an abstract class.
  */
class ICAPI
{
public:
